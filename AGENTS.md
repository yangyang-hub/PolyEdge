...
- Rewards bot 的 `quote_bid_rank` 仅允许 `1`、`2`、`3`，分别表示按盘口不同买价挂在买一、买二、买三，默认 `1`；所选档位不是相对中间价偏移，目标档位不足时不会回退到其他价格。该检查只在 live placement 准备挂单时基于当前 orderbook 执行，不在 quote plan 构建阶段提前过滤候选；缺少/过期盘口会保持等待订阅数据返回，非 transient 验证失败才写入 12 小时 `live_skip_until`/`live_skip_reason`。
...
- Rewards bot 的 `per_market_usd` 是 YES + NO 两腿合计预算：live materializer 先满足报价腿按 CLOB 成本精度向上对齐后的 `rewards_min_size`，再按单腿目标 notional 缺口分配剩余额度，不再固定均分预算而误拒绝价格不对称市场；`quote_mode=auto` + `selection_mode=enforce` + dominant single-side 开启时，双边最小份额预算不足会按当前实际单腿价格尝试单边回退。
- `POLYEDGE_ORDERBOOK_STREAM__MAX_TOKENS` 默认 3000；调高会增加 orderbook WS/poll 内存占用，调低会减少 rewards 盘口覆盖。订阅预算依次分配给活跃 rewards、execution、当前 eligible rewards 和其余候选 token；rewards full tick 会按需把通过 gate 的 eligible quote plan token 注册到 `rewards_eligible`。`POLYEDGE_ORDERBOOK_STREAM__REWARD_CANDIDATE_TOKEN_CAP` 默认 100，只限制 rewards 候选预热 source，不影响活跃订单、持仓、execution 或 eligible quote plan token；设为 0 可关闭候选预热以快速降带宽，但 eligible 市场仍会按需订阅。`POLYEDGE_ORDERBOOK_STREAM__WS_CHUNK_SIZE` 默认 250，用于控制每条 Polymarket WS 连接承载的 token 数；调低会减少单连接消息压力、增加连接数，调高则相反。`POLYEDGE_ORDERBOOK_STREAM__MAX_LEVELS_PER_SIDE` 默认 100，用于限制进程内缓存和 HTTP ingest 每个 token 的 bids/asks 保留深度；写入时先排序再裁剪，保留最优档位。poll 每周期会刷新全部注册 token；`POLYEDGE_ORDERBOOK_STREAM__STALE_THRESHOLD_MS=0` 只关闭年龄 stale 优先级。
