...
- Rewards bot 的 `per_market_usd` 是 YES + NO 两腿合计预算：报价计划先满足两腿按 CLOB 成本精度向上对齐后的 `rewards_min_size`，再按单腿目标 notional 缺口分配剩余额度，不再固定均分预算而误拒绝价格不对称市场；`quote_mode=auto` + `selection_mode=enforce` + dominant single-side 开启时，双边最小份额预算不足会按实际单腿价格尝试单边回退。
