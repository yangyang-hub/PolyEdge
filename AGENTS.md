...
| `packages/backend/init.sql` | Complete empty-database initialization script generated from migrations 0001–0046 |
...
- 数据库迁移目前到 `0046_reward_low_competition_competition_share.sql`；`packages/backend/init.sql` 是按 0001–0046 合并的空库完整初始化脚本，运行时仍保留 `packages/backend/migrations/` 给 `sqlx` 校验和增量迁移使用。
