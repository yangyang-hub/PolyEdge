...
- Rewards bot `auto_cancel_stale_minutes` 默认 10；超过此时间的卡死对账订单（404、提交结果未知、取消对账未完成等）会被自动 force-cancel 并解除新买单阻塞；设为 0 关闭自动清理。
