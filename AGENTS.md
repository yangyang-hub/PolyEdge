...
| `packages/backend/crates/application/src/rewards/runtime_models.rs` | Rewards runtime models — account/position/order/fill/merge-intent/event/report/snapshot and LLM usage types |
...
| `packages/backend/crates/application/src/rewards/config_impl.rs` | Rewards config defaults、normalization、standard/BalancedMerge candidate filter and patch application |
...
| `packages/front/src/features/rewards/components/rewards-config-panel.tsx` | Rewards config panel — 执行、市场筛选、机会评分、报价构造、成交后合并、AI/信息风险和库存控制 |
...
| `packages/front/src/features/rewards/components/rewards-tables.tsx` | Rewards tables — quote plan/订单/持仓/成交/事件展示，显示 standard/BalancedMerge profile、AI/info-risk 二值决策和 AI direction/rank/notional hint |
...
| `packages/backend/init.sql` | Complete empty-database initialization script generated from migrations 0001–0058 |
...
- 数据库迁移目前到 `0058_reward_market_fair_values.sql`；`packages/backend/init.sql` 是按 0001–0058 合并的空库完整初始化脚本，运行时仍保留 `packages/backend/migrations/` 给 `sqlx` 校验和增量迁移使用。
