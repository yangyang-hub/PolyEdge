...
| `packages/backend/crates/connectors/src/openai_compat.rs` | OpenAI-compatible provider helper — root base URL 自动补 `/v1`、已带 `/vN` 的 provider base 保持原样，Bearer + `api-key` 认证头兼容，模型名包含 GLM/DeepSeek 时的 Chat Completions JSON mode 差异，glm-4.7 思考模型注入 `thinking:{type:disabled}`，provider 文本响应候选 JSON 提取 |
| `packages/backend/crates/connectors/src/reward_provider.rs` | Rewards combined provider connector — 单市场一次 LLM 调用可同时返回 AI advisory 与 info-risk，支持 OpenAI Responses、OpenAI-compatible Chat Completions（含 GLM/DeepSeek 模型名特例）、Anthropic Messages 和可选 OpenAI Responses web search；glm-4.7 思考模型在 Chat Completions 路径关闭 thinking 并把 completion token 预算设为 8192，避免 reasoning_content 烧光预算导致 `content` 空被 `max_tokens` 截断（`finish_reason: length`） |
...
- GLM `https://open.bigmodel.cn/api/coding/paas/v4` + `glm-4.7` 和 DeepSeek `https://api.deepseek.com` + `deepseek-v4-flash` 已用测试 key 对当前 Chat Completions 请求形态做过鉴权 smoke test（均 HTTP 200，返回可解析 JSON object 字符串）；测试 key 不写入仓库，正式运行仍只通过 worker 环境变量注入。glm-4.7 系列（含 `glm-4.7-flashx`）默认开启思考，rewards combined provider 的 Chat Completions 请求会注入 `thinking:{type:disabled}` 并把 completion token 预算设为 8192，避免思考链烧光 token 预算导致 `content` 为空被 `max_tokens` 截断（`finish_reason: length`）；Smart Money signal advisory 仍是 2048 预算且未注入 thinking。
