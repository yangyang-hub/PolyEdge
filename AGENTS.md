...
| `packages/backend/apps/worker/src/worker/rewards/provider_advisory.rs` | Rewards AI advisory cache gate, pre-LLM candidate hard filter, combined provider connector/permit helpers and LLM call recording helper |
...
| `packages/backend/apps/worker/src/worker/rewards/info_risk.rs` | Rewards info-risk async scan loop, provider cache lookup/write, pre-LLM candidate hard filter, quote-plan risk application（按当前 request/input_hash 精确应用缓存并清理旧 risk 展示）, provider call recording |
...
- Rewards snapshot 包含 `llm_usage`，按 UTC 日展示 combined provider 的真实外部请求总数、失败请求数以及 AI advisory / info-risk section 计数；同一请求同时携带 advisory 和 info-risk 时总数只加 1、两个 section 计数各加 1。前端 `/rewards` 顶部执行概览展示今日真实外部请求总数和最近 7 天明细，并明确 AI/风控列是 section 计数，统计不包含 provider 缓存命中。Rewards combined provider 和 Smart Money signal advisory 的实际外部大模型 HTTP 调用在 connectors 层共享单进程全局 semaphore，同一进程内任意时刻最多 1 个大模型请求在飞。
