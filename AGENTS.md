...
| `packages/backend/init.sql` | Complete empty-database initialization script generated from migrations 0001–0056 |
...
- Rewards BalancedMerge 成交后合并 profile 默认关闭；开启后使用独立 `balanced_merge_*` 市场/订单/edge/质量/未配对库存阈值追加低活跃度候选，同一 condition 若已被 standard 选中则 standard 优先。该 profile 固定 YES/NO 双边 BUY，要求两侧买价合计满足 edge；成交后不生成 SELL 退出、不撤对侧 BUY；full tick 和 fast reconcile 会自动发现同 condition 已有 YES/NO 可配对库存，扣除 active merge intent 后写入 `RewardMergeIntent(status=unsupported)` 防重。BalancedMerge BUY 忽略全局 `cancel_bid_rank` 档位撤单限制，但仍受盘口缺失/过期、深度、事件窗口、kill switch 等硬风控约束。当前没有自动链上 CTF merge connector，不会实际合并或把 intent 标为完成。
...
- 数据库迁移目前到 `0056_reward_managed_orders_external_inventory.sql`；`packages/backend/init.sql` 是按 0001–0056 合并的空库完整初始化脚本，运行时仍保留 `packages/backend/migrations/` 给 `sqlx` 校验和增量迁移使用。
...
- Rewards BalancedMerge 当前只完成候选/profile 分流、挂单/撤单/last-look 兼容、成交后不退出和 merge intent 持久化；自动执行链上 CTF merge/redeem 尚未实现，`reward_merge_intents.status=unsupported` 只是待处理意图记录。
