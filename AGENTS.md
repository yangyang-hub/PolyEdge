...
| `packages/backend/apps/worker/src/worker/rewards/live_orders.rs` | Rewards live cancel/fill, external-order cancel in-flight dedupe, post-fill exit/flatten intents, and BalancedMerge merge intents |
...
- Rewards BalancedMerge 成交后合并 profile 默认关闭；开启后使用独立 `balanced_merge_*` 市场/订单/edge/质量/未配对库存阈值追加低活跃度候选，同一 condition 若已被 standard 选中则 standard 优先。该 profile 固定 YES/NO 双边 BUY，要求两侧买价合计满足 edge；成交后不生成 SELL 退出、不撤对侧 BUY，等 YES/NO 库存可配对后只写入 `RewardMergeIntent(status=unsupported)` 防重。当前没有自动链上 CTF merge connector，不会实际合并或把 intent 标为完成。
