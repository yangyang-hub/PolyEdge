...
Orderbook cache-only reads remain unauthenticated, but positive `refresh_if_stale_ms` batches and the internal `/orderbook/stream` require the shared orderbook write token. Upstream CLOB batch work has a 20-second per-batch deadline; internal stream connections are capped and slow sends time out.
...
- Fair value now blends YES/NO midpoint parity with top-of-book microprice imbalance and history, and includes both dynamic market uncertainty and AI edge buffer in the final edge gate. If the first nominal rank fails the robust estimate, the planner searches deeper configured ranks before blocking. Market selection weights are 15% base quality, 10% reward density, 20% fair-value edge, 30% exit quality and 25% stability; a full edge score requires 4 cents because the estimate remains market-implied rather than independent alpha.
