...
| `packages/backend/crates/application/src/rewards/replay.rs` + `replay_v2.rs` | V1/V2 deterministic replay, compact top-of-book history, final delta and expected-plan hashes |
...
| `packages/backend/apps/worker/src/worker/rewards/replay_capture.rs` | Bounded asynchronous Replay V2 capture writer and shutdown drain |
...
- Database schema uses a frozen clean-deploy baseline plus forward fixes: `0001_initial_schema.sql` must never be edited after release, `0002_reward_fair_value_history_identity.sql` upgrades existing deployments, and `packages/backend/init.sql` represents the latest complete schema. Historical migrations for removed modules remain gone.
