...
- Rewards 低竞争市场 sleeve 已有 observe/enforce v2、独立小额度配置、指标 gate、跨周期 observation 和策略观察面板；低竞争候选强制不使用 liquidity/24h volume 作为准入门槛，旧低竞争 liquidity/volume 过滤字段兼容读取但归一化后关闭并置 0；前端 quote plan 行内按低竞争硬性通过状态和逐项门槛展示策略情况；低竞争待提交 BUY 的 last-look 会强刷并要求全计划 token，挂单后常规撤单和事件驱动 hard-risk 撤单会补齐必要盘口并走低竞争专用撤单 gate，不再复用普通市场 depth/rank/history/requote 撤单规则。仍缺自动启用/自动切换 enforce。
