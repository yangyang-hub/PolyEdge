...
Orderbook WS chunk 在 client 创建或初始 subscribe 失败时会按 per-chunk backoff 重试（约 2 秒起步，最高 60 秒），退避等待期间仍会处理 shutdown/reconcile 命令，避免 Polymarket/Cloudflare 502/522 时快速重连刷屏；已有连接的 reader 正常结束仍只重建对应 chunk，poll reconciler 和 HTTP batch refresh 继续兜底新鲜度。
...
| `packages/backend/crates/connectors/src/polymarket/chain.rs` | Polygon chain connector — 读取资金钱包链上 pUSD ERC20 余额；通过 Polymarket Bridge 生成入金地址并广播后端资金钱包 USDC/USDT 转账；Safe proxy CTF mergePositions 提交 |
...
| `packages/backend/crates/connectors/src/openai_compat.rs` | OpenAI-compatible provider helper — root base URL 自动补 `/v1`、已带 `/vN` 的 provider base 保持原样，Bearer + `api-key` 认证头兼容，模型名包含 GLM/DeepSeek 时的 Chat Completions JSON mode 差异，Agnes Chat Completions 端点归一化，glm-4.7 思考模型注入 `thinking:{type:disabled}`，provider 文本响应候选 JSON 提取 |
| `packages/backend/crates/connectors/src/reward_provider.rs` | Rewards combined provider connector — 单市场一次 LLM 调用可同时返回 AI advisory 与 info-risk，支持 OpenAI Responses、OpenAI-compatible Chat Completions（含 GLM/DeepSeek/Agnes 模型名特例）、Anthropic Messages 和可选 OpenAI Responses web search；glm-4.7 思考模型在 Chat Completions 路径关闭 thinking 并把 completion token 预算设为 8192，避免 reasoning_content 烧光预算导致 `content` 空被 `max_tokens` 截断（`finish_reason: length`） |
| `packages/backend/crates/connectors/src/reward_ai.rs` | Rewards AI advisory provider schema/parse helper — 二值 `allow_quote`、conservative `strategy_hint` 与旧 `suitability` fail-closed 兼容 |
| `packages/backend/crates/connectors/src/reward_ai_tests.rs` | Rewards AI advisory parse tests — 二值 `allow_quote`、`strategy_hint` 和旧 `suitability` 响应兼容 |
| `packages/backend/crates/connectors/src/reward_info_risk.rs` | Rewards info-risk provider schema/parse helper — 二值 `allow_quote` 与旧 taxonomy response 兼容 |
| `packages/backend/crates/connectors/src/reward_info_risk_tests.rs` | Rewards info-risk parse tests — 二值 `allow_quote`、旧响应兼容和 confidence clamp |
| `packages/backend/crates/connectors/src/smart_signal_advisory.rs` | Smart Money signal advisory connector — OpenAI Responses、OpenAI-compatible Chat Completions（含 GLM/DeepSeek/Agnes 模型名特例）和 Anthropic Messages，解析 `allow/observe/reject` 三态建议 |
...
| `packages/backend/apps/worker/src/worker/rewards.rs` | Rewards bot — executes live strategy ticks and queued run/cancel/reset commands；full tick 会在机会评分后应用默认关闭的做市商 shadow/guarded EV 决策并记录审计 |
...
| `packages/backend/apps/worker/src/worker/rewards/provider_advisory.rs` | Rewards AI advisory cache gate, pre-LLM candidate hard filter, combined provider connector/concurrency helpers and LLM call recording helper |
| `packages/backend/apps/worker/src/worker/rewards/provider_fallback.rs` | Rewards LLM provider fallback — 解析可选第二个独立 provider/model 接口、主接口任意失败时用同一 combined request 重试备用接口（`evaluate_with_fallback`）、合并 overload 判定、primary/fallback 双 tuple 缓存读取 |
| `packages/backend/apps/worker/src/worker/rewards/provider_refresh_orderbook.rs` | Rewards provider refresh orderbook helper — 先复用本轮 books 和 orderbook HTTP batch stale refresh 准备 AI 所需盘口，仅仍缺盘口时才用 `rewards_ai_provider` 临时 source 订阅缺口 token |
| `packages/backend/apps/worker/src/worker/rewards/provider_refresh.rs` | Rewards combined provider refresh — full tick 应用已有 provider cache gate 后启动单个后台 task，按统一候选顺序和 Rewards 配置的主/备 provider 并发限制补齐到期 AI advisory/info-risk section；provider refresh 只写 advisory/info-risk 缓存，不直接改写 `rewards_eligible`；实际 provider 调用写入 `llm_calls(task_type=reward_provider)` |
...
| `packages/backend/apps/worker/src/worker/rewards/info_risk.rs` | Rewards info-risk async scan loop, provider cache lookup/write, pre-LLM candidate hard filter, Rewards provider concurrency limit, quote-plan risk application（按当前 request/input_hash 精确应用缓存并清理旧 risk 展示）, provider call recording |
...
| `packages/backend/crates/application/src/rewards/runtime_models.rs` | Rewards runtime models — account/position/order/fill/merge-intent/event/report/snapshot、quote plan market-maker diagnostics and LLM usage types |
| `packages/backend/crates/application/src/rewards/quote_selection_models.rs` | Rewards quote/selection/AI advisory/strategy-mode enums — double/auto、observe/enforce、rewards_only/market_maker_shadow/market_maker_guarded、provider/request format |
...
| `packages/backend/crates/application/src/rewards/config_impl.rs` | Rewards config defaults、normalization、standard/BalancedMerge/market-maker candidate filter and patch application |
| `packages/backend/crates/application/src/rewards/planner.rs` | Rewards deterministic quote planner — 静态事件风险过滤、首单 info-risk/quarantine gate、strategy profile quote plan 构建 |
...
| `packages/backend/crates/application/src/rewards/market_maker.rs` | Rewards market-maker EV gate — 读取 fair value + opportunity metrics 计算 pricing/reward/exit/adverse/inventory/uncertainty EV，shadow 只记录诊断，guarded 可保守重定价/过滤 BUY quote legs |
...
| `packages/backend/crates/application/src/rewards/planner_live.rs` | Rewards live quote materializer — live orderbook rank/rewards-spread/live-token-spread/auto metrics and AI-hinted conservative rank before wallet-balance placement gate |
...
| `packages/backend/crates/infrastructure/src/stores/rewards/postgres.rs` | Rewards Postgres store — 完整 rewards 持久化、market-maker fair value read/decision audit、`llm_calls` 记录和每日调用统计 |
...
| `packages/backend/crates/infrastructure/src/stores/rewards/postgres_market_maker.rs` | Rewards market-maker Postgres store — 读取 `reward_market_fair_values` 最新快照并 upsert `reward_market_maker_decisions` 审计 |
| `packages/front/src/features/rewards/components/rewards-overview-cards.tsx` | Rewards overview cards — live status, daily LLM usage（总数为真实外部请求，AI/风控为 section 计数）, command center and summary metrics |
| `packages/front/src/features/rewards/components/rewards-config-panel.tsx` | Rewards config panel — 执行、市场筛选、机会评分、做市商 EV、报价构造、成交后合并、AI/信息风险和库存控制 |
...
| `packages/front/src/features/rewards/components/rewards-market-maker-summary.tsx` | Rewards market-maker summary — quote plan 做市商 EV、fair value、目标价和阻断原因诊断 |
| `packages/front/src/features/rewards/components/rewards-advanced-config.tsx` | Rewards advanced config — 盘口选择、AI advisory strategy hint、做市商 fair-value/EV/inventory/TTL 配置、信息风险和事件窗口配置 |
| `packages/front/src/features/rewards/components/rewards-tables.tsx` | Rewards tables — quote plan/订单/持仓/成交/事件展示，显示 standard/BalancedMerge profile、AI/info-risk 二值决策、AI direction/rank/notional hint 和 market-maker diagnostics |
...
| `packages/front/src/features/high-probability/components/high-probability-workbench.tsx` | High Probability workbench — 只读展示研究配置、research report、baseline backtest、基础退出规则对比、bucket stats、fair value 定价表和 observations，不触发交易 |
| `packages/front/src/features/high-probability/components/high-probability-fair-values.tsx` | High Probability fair value 定价表 — 只读展示 `reward_market_fair_values` 快照（fair_yes 区间/中值、置信度、不确定性、回退层级、可上线徽章和原因） |
...
| `packages/front/src/features/high-probability/loaders/high-probability-page-data.ts` | High Probability frontend loader — 通过前端数据层读取 `/api/v1/high-probability` snapshot、`/report`、`/backtests`、`/backtest-runs` 和 `/fair-values` |
| `packages/front/src/features/high-probability/lib/high-probability-formatters.ts` | High Probability frontend formatters — 概率、回撤、bucket 维度、report note、exit rule、fair value side/fallback/eligible 和空值展示 helper |
| `packages/front/src/lib/api/high-probability.ts` + `packages/front/src/lib/contracts/dto/high-probability.ts` | High Probability frontend data layer — 只读读取 snapshot/config/bucket stats/report/backtest/exit-rule/backtest-runs/fair-values DTO（含 `HighProbabilityFairValueDto`） |
...
| `packages/backend/apps/worker/src/worker/rewards/account_sync.rs` | Rewards external balance, CLOB open-order snapshot/adoption, external active SELL token coverage, complete position snapshot sync, and detected-inventory original-price sell intents |
| `packages/backend/apps/worker/src/worker/rewards/live_orders.rs` | Rewards live cancel/fill, external-order cancel in-flight dedupe, post-fill exit/flatten intents, and BalancedMerge fill/position merge-intent discovery |
...
| `packages/backend/apps/worker/src/worker/rewards/live_pending.rs` | Rewards durable intent submit/recovery workflow — BUY 提交前 last-look 刷新当前 plan token 集合、重新 materialize 并在 POST 前按 live 风控重定价或取消本地 intent |
...
| `packages/backend/apps/worker/src/worker/rewards/live_cancel.rs` | Rewards live cancel reason routing — 通用硬风控、live token spread、depth/rank/history/requote 规则和 strategy profile plan 匹配 |
| `packages/backend/apps/worker/src/worker/rewards/live_risk.rs` | Rewards live placement risk checks and shared risk helpers, including BUY live token spread cancellation |
...
| `packages/backend/apps/worker/src/worker/rewards/event_cancel.rs` | Rewards orderbook event-driven hard-risk cancel worker — 独立消费活跃 token 更新并立即 cancel-only 撤单，使用统一 hard-risk/live-spread/depth/rank/history gate |
...
| `packages/backend/apps/worker/src/worker/high_probability.rs` | High Probability Pricing worker — 导入本地 outcome JSON 标签，从本地 outcome 标签 + rewards candles 构建 first-touch 样本（含研究特征）、刷新动态高概率策略 bucket stats、持久化 baseline backtest run/trade/退出规则摘要，用 orderbook 服务缓存执行一次性只读 observe 扫描，并把 bucket stats + 盘口融合成保守 fair value 快照 upsert 到 `reward_market_fair_values`；不抓外部 API、不交易 |
...
| `packages/backend/api/src/handlers/high_probability.rs` | High Probability Pricing API — research snapshot/config/bucket stats/report/backtests/backtest-runs/fair-values；handler 只读写 HighProbabilityService，不抓外部 API、不执行交易 |
...
| `packages/backend/crates/application/src/smart_money.rs` + `smart_money/*` | SmartMoneyService/Store models — Smart Money config（含 signal advisory provider/request-format/model 与并发限制）、候选钱包、候选状态更新、画像、确定性评分、源交易、确定性 signal gate、signal decision 审计、signal advisory 缓存、signal advisory request payload/input_hash builder、provider decision 模型、worker refresh 所需只读列表代理和 snapshot 基础模型 |
...
| `packages/backend/migrations/0055_reward_balanced_merge_strategy.sql` | Rewards BalancedMerge profile schema — `strategy_profile` columns and `reward_merge_intents` |
| `packages/backend/migrations/0056_reward_managed_orders_external_inventory.sql` | Rewards external inventory managed-order schema — remove `reward_managed_orders.condition_id` FK for non-catalog inventory exits |
| `packages/backend/migrations/0057_reward_merge_intent_execution.sql` | Rewards BalancedMerge execution schema — merge intent tx hash、提交/确认/失败时间、失败原因与 retry count |
| `packages/backend/migrations/0058_reward_market_fair_values.sql` | High Probability fair value provider schema — `reward_market_fair_values` 保守 fair_yes 区间/置信度/不确定性/reason_codes/live_eligible 快照 |
| `packages/backend/migrations/0059_reward_market_maker_decisions.sql` | Rewards market-maker decision audit schema — `reward_market_maker_decisions` 记录 shadow/guarded EV、fair value 引用、目标报价和原因码 |
| `packages/backend/init.sql` | Complete empty-database initialization script generated from migrations 0001–0059 |
...
- Rewards snapshot 包含 `llm_usage`，按 UTC 日展示 combined provider 的真实外部请求总数、失败请求数以及 AI advisory / info-risk section 计数；同一请求同时携带 advisory 和 info-risk 时总数只加 1、两个 section 计数各加 1。前端 `/rewards` 顶部执行概览展示今日真实外部请求总数和最近 7 天明细，并明确 AI/风控列是 section 计数，统计不包含 provider 缓存命中。大模型 HTTP 调用不再由 connectors 层全局单飞；Rewards 使用前端配置保存的主/备 provider 并发池，Smart Money signal advisory 使用 Smart Money 独立并发池，高概率策略当前没有 LLM provider 调用路径。
...
- Rewards 统一机会评分默认启用，当前基线为 10U 探针、100U 日奖最低 0.75、竞争倍数软上限 4、竞争倍数硬门槛默认开且阈值 1000x（超过即不可挂，区别于软上限仅 warning）、账户/单市场占用警告 1500/500 bps、退出深度至少 60U 或计划名义额 2.5 倍、入场退出滑点 2c、坏成交恢复 3 天、30 分钟观察窗口至少 30 个盘口样本、中点波动 3c、top-of-book 跳变 8 次，评分权重为 reward/competition/exit/stability = 35/30/25/10。
- Rewards 做市商策略已完成初始落地但默认关闭：`strategy_mode=rewards_only` 且 `market_maker_enabled=false`。开启 `market_maker_shadow` 时仅读取 `reward_market_fair_values` 和本轮 `opportunity_metrics` 计算 pricing/reward/exit/adverse-selection/inventory/uncertainty EV，并写入 `reward_market_maker_decisions` 与 quote plan `market_maker` 诊断，不影响 live 挂单；开启 `market_maker_guarded` 时只在已有 deterministic BUY quote plan 之上保守重定价/过滤报价腿，仍受 AI/info-risk、事件窗口、盘口新鲜度、资金、库存、kill switch 和 live 风控约束。该版本不包含完整 SELL 管理器、EV 驱动撤单、PnL 归因或校准/漂移监控，不能描述为全量做市系统。
- Rewards BalancedMerge 成交后合并 profile 默认关闭；开启后使用独立 `balanced_merge_*` 市场/订单/edge/质量/未配对库存阈值追加低活跃度候选，同一 condition 若已被 standard 选中则 standard 优先。该 profile 固定 YES/NO 双边 BUY，要求两侧买价合计满足 edge；成交后不生成 SELL 退出、不撤对侧 BUY；full tick 和 fast reconcile 会自动发现同 condition 已有 YES/NO 可配对库存，扣除 active merge intent 后写入 `RewardMergeIntent` 防重。`balanced_merge_auto_execute_enabled` 默认关闭，关闭时 intent 为 `unsupported`；开启后 intent 为 `pending`，worker 会用配置私钥作为 Safe owner 通过 proxy wallet 提交 CTF `mergePositions`，提交成功写入 tx hash 并标记 `submitted`，提交前失败标记 `failed`。BalancedMerge BUY 忽略全局 `cancel_bid_rank` 档位撤单限制，但仍受盘口缺失/过期、深度、事件窗口、kill switch 等硬风控约束。
- GLM `https://open.bigmodel.cn/api/coding/paas/v4` + `glm-4.7`、DeepSeek `https://api.deepseek.com` + `deepseek-v4-flash` 和 Agnes `https://apihub.agnes-ai.com/v1` + `agnes-2.0-flash` 沿用 OpenAI-compatible provider 配置；GLM/DeepSeek 已用测试 key 对当前 Chat Completions 请求形态做过鉴权 smoke test（均 HTTP 200，返回可解析 JSON object 字符串），Agnes 已用测试 key 对 Chat Completions strict JSON schema、Rewards combined schema 和 Smart Money signal advisory schema 做过 smoke test（均 HTTP 200，返回可解析 JSON）。测试 key 不写入仓库，正式运行仍只通过 worker 环境变量注入。模型名包含 `glm` 或 `deepseek` 时强制 Chat Completions 并使用 `response_format=json_object` + `max_tokens`；模型名包含 `agnes` 时也强制 Chat Completions，但保留 strict JSON schema + `max_completion_tokens`。glm-4.7 系列（含 `glm-4.7-flashx`）默认开启思考，rewards combined provider 的 Chat Completions 请求会注入 `thinking:{type:disabled}` 并把 completion token 预算设为 8192，避免思考链烧光 token 预算导致 `content` 为空被 `max_tokens` 截断（`finish_reason: length`）；Smart Money signal advisory 仍是 2048 预算且未注入 thinking。
...
- Rewards 首次买入报价现在有可配置入场缓冲：当 `info_risk_enabled=true` 且 `info_risk_mode=enforce` 时，`require_info_risk_before_first_quote=true` 会要求新 condition 先命中未过期信息风险缓存，`first_quote_quarantine_sec` 会要求新 condition 至少观察指定秒数（默认 600）后才允许首次 live BUY；首单观察起点持久化在 quote plan JSON 的 `first_quote_observed_at`，每轮 rebuild 会从上一版 quote plan 继承该字段，避免 funding/provider/live gate 更新 `updated_at` 时重置观察窗口；已有 open-like 订单或持仓的 condition 跳过该首单 gate，以便继续撤单、退出和管理库存。planner 的静态事件风险关键词也扩展到 appointed/confirmed/certified、drop out/withdraw/resign/step down/removed、scheduled/deadline/market-close 以及地缘/军事事件（airspace closure、ceasefire、military strike、nuclear test、martial law、state of emergency 等）类市场；Gamma 富分类未传播到 rewards 候选（全部 category="Polymarket"），所以地缘事件市场只能靠关键词静态过滤。
- Rewards full tick 在订单/账户同步、live action 盘口刷新、quote readiness materialize 和 opportunity metrics 更新完成后，先保存最终 quote plan 快照，再用同一最终口径替换 `rewards_eligible` orderbook source；周期注册任务读取同一持久化口径作为兜底，避免中间态和最终状态反复覆盖同一 source。
- Rewards 成交后退出策略由 `post_fill_strategy` 配置决定，不再由 AI advisory 的 `exit_policy` 覆盖；`ExitAtMarkup` 以被吃买单原价加 `exit_markup_cents` 为卖价基准并向上取整到 0.01 tick，默认加价为 0；页面“持有并续挂”对应 `hold_and_requote`，也会按被吃买单原价持久化 SELL 退出 floor，之后继续正常报价。`ExitAtMarkup`、`HoldAndRequote` 和外部库存补退出按 floor 提交 post-only maker SELL，当前买一穿过 floor 时改挂当前卖一，不递延也不 taker 出；只有缺少可 resting 卖一或盘口异常时才按 30 秒退避等待。`FlattenImmediately` 在 best bid 不低于 floor 时走非 post-only FAK/taker SELL，否则按 30 秒退避等待非亏损 bid。SELL 提交前会按当前 token 持仓裁剪 size，无持仓 stale exit 会关闭，不使用 midpoint 或页面“当前价”降价。
- Rewards 外部 positions 快照检测到已有库存且没有同 token open-like SELL、也没有同 token 外部 active SELL 时，会按持仓 `avg_price` 向上对齐 tick 后补原价 SELL intent；外部库存可能来自当前 rewards catalog 之外的 condition，`reward_positions.condition_id` 和补退出写入的 `reward_managed_orders.condition_id` 都不依赖 `reward_markets` 外键；后续按 post-only maker 规则挂出，买一已高于/等于 floor 时改挂当前卖一，避免已有库存无人接管退出且避免用低于持仓均价的买一价或 midpoint 卖出。
...
- Rewards CLOB open-order snapshot 会先收养未归属但 token 可唯一映射到 active reward market 的开放 BUY 为 managed order；如果同 external id 的本地 BUY 已被关成非 open，但 CLOB 仍 open，会重开原本 managed order。外部 active SELL 不接管为 managed order，但会按 token 覆盖外部库存补 SELL intent，避免官网/其它机器人已挂卖单时重复补单；非 rewards 市场和无法唯一映射 token 的外部开放订单明细，以及奖励结算对账仍是缺口。
...
- Rewards AI advisory / info-risk provider refresh 受 `POLYEDGE_REWARDS__INFO_RISK_MAX_MARKETS_PER_CYCLE` 控制（默认 50，0 表示本轮不发 provider 请求），该值限制本轮真实外部 provider 请求数，缓存命中不占名额。full tick 会先执行 live funding precheck，再记录 AI 过滤前 deterministic eligible condition 集合；live tick 只读取已有 advisory/info-risk 缓存并 fail closed，不等待外部 provider。后台 provider refresh 仍由单个 Tokio task 和 `AtomicBool` 防止同一进程重复 refresh，但实际 provider 请求按 `RewardBotConfig.ai_provider_concurrency_enabled`、`ai_provider_primary_max_concurrency` 和 `ai_provider_fallback_max_concurrency` 分别限制主/备 endpoint 并发；关闭并发时主备都保持 1 并发，开启后各自钳制为 1–10。候选按开放订单、持仓优先，其余按统一 standard 顺序处理；refresh 会继续扫描候选直到请求额度耗尽、候选耗尽、遇到停止型 provider 错误或 wall-clock timeout，完成日志中的 `provider_checked` 表示实际检查候选数，`provider_request_cap_exhausted=true` 表示因真实请求额度用完而停止。每个 condition 最多发起一次 combined provider 调用，请求只携带已启用且到期的 `advisory` / `info_risk` section，成功后分别写入 `reward_market_advisories` / `reward_market_info_risks`。AI advisory section 仍要求该市场所有报价 token 已有非空 bids/asks，refresh 会先复用本轮 books 并通过 orderbook HTTP batch stale refresh 获取盘口，仅仍缺的 token 才临时注册到 `rewards_ai_provider` source，结束或超时后清空；info-risk-only section 不要求盘口。新 provider 输出 `allow_quote=true|false` 二值和 conservative `strategy_hint`，旧 `suitability` 响应按二值 fail-closed 兼容；缓存使用各自稳定 input hash 和确定性 TTL jitter，进入 `min(TTL/20, 60s)` 提前刷新窗口时才续期。实际外部调用记录为 `llm_calls.task_type=reward_provider`，`parsed_output` 同时包含返回的 advisory/info_risk section，HTTP/解析失败只影响统计和 fail-closed/缓存策略；可选 fallback 会用同一 combined request 重试备用 endpoint。只有 HTTP 429/401/403/5xx、明确 overloaded 或 timeout/timed out 才停止本轮剩余 provider 请求；普通连接重置/DNS/TLS/响应流中断会继续下一个候选，由本轮 wall-clock timeout 兜底。
...
- Rewards AI advisory / info-risk provider 请求前统一应用 pre-LLM 硬过滤：有开放订单或持仓的 condition 始终保留最高优先级；无敞口计划必须仍是 eligible 或 pre-AI eligible；legacy low-competition bucket 按 standard 候选处理，不再形成独立候选类型或观察队列。该过滤同时覆盖 full tick combined provider refresh 和 AI 未启用时的独立 info-risk scanner，避免 market-only 候选变成大模型调用。
- Rewards combined provider 和 Smart Money signal advisory 的实际外部 provider HTTP 调用会写入 `llm_calls`；Rewards combined provider 的 `task_type=reward_provider`，Smart Money 为 `smart_signal_advisory`。HTTP/解析失败会记录为失败调用，失败记录只影响统计和排查，不改变原有 fail-closed/缓存策略。
- Rewards live 撤单已回到统一路径：BUY/SELL 都共享通用硬风控、depth/rank/history/requote、best ask touch、kill switch、post-only/cancel retry 和对账锁检查；BUY 还会在实时 token `best_ask - best_bid` 超过 `max_market_spread_cents` 时撤单；不再有低竞争专用撤单 gate、companion token 复核或独立宽松阈值。
- Rewards info-risk provider payload 会携带 `evaluation_time_utc` 和 imminent 判定策略；Chat Completions、Anthropic Messages 和 Responses prompt 都要求按该 UTC 时间判断 `resolution_imminent`，避免 provider 使用训练日期或过期上下文把远期/历史事件误判为临近结算。info-risk cache key `schema_version=7` 会让旧多状态、未携带 provider_cache_policy、旧低竞争策略字段、仍包含 `quote_mode`/`recommended_quote_mode` 或仍包含 `event_slug`/`ambiguity_level` 的缓存失效并重新评估；`evaluation_time_utc` 不进入稳定 cache key，避免每轮刷新造成缓存雪崩。info-risk 与 AI advisory cache key 都已剔除每轮易变的 materialized `quote_mode`/`recommended_quote_mode`（advisory `schema_version=11`）；info-risk 还额外剔除了 `event_slug`（来自 rewards catalog 同步）和 `ambiguity_level`（来自 Gamma 同步），这两个字段由两个独立 sync loop 写入、跨周期会漂移，留在 key 里会让缓存查不到、把 enforce 模式下的 eligible 计划间歇性打成 pending（最终可挂市场周期性归零）。info-risk 现在只用与 advisory 一致的稳定市场身份字段（condition_id/question/market_slug/category/end_at）。
...
- Rewards live 会在提交旧 intent 前先执行当前盘口/资格撤单检查；BUY intent 在真正 POST 前还会按当前 quote plan token 集合请求 orderbook 服务 1 秒 max-age books，重新 materialize 当前 plan 后改价或取消；盘口缺失、刷新失败、live token spread 超过 `max_market_spread_cents` 或出现 best ask touch 等硬风险则不提交。任一提交结果未知、待最终对账或外部订单 404 会暂停全部新增买单，但继续同步、撤单和卖出退出；外部订单 404 锁超过 5 分钟且仍无成交证据时会自动本地关闭。提交结果未知时，开放订单严格匹配失败会先保持对账锁；当恢复查询确认 CLOB 无对应 open 订单并经过 `LIVE_SUBMISSION_UNKNOWN_CLOSE_AFTER_SECS`（默认 600 秒）宽限后，worker 会把该订单本地标记为 cancelled 以释放锁（与 404 锁一致），但若 positions 快照显示该 BUY token 在提交后出现库存则继续保留对账锁。CLOB `post_order` 只要返回订单 ID 就保留为 accepted 供后续成交/状态对账，包含 `unmatched` / `canceled` / 未知状态；HTTP 4xx 明确拒单会标记当前 intent 为 error，只有网络中断、5xx 或成功响应缺少订单 ID 才进入提交结果未知锁。managed order 的后续 upsert 会同步更新实际提交数量；SELL intent price 保留非亏损退出 floor；post-only exit 被取消后的重试保留退出 floor，若 floor 会穿当前买一则实际提交价改用当前卖一，flatten replacement 保留退出 floor 并在后续按 best bid 非亏损 FAK 或继续等待。订单 scoring 观测只推进 `last_scored_at`，不修改业务状态 `updated_at`；reconciliation 锁订单跳过 scoring 查询，避免周期性观测掩盖真实业务状态年龄。
...
- Rewards BUY last-look 当前按 quote plan token 集合请求 orderbook 服务 1 秒 max-age books，并用刷新 books 重新 materialize 当前 plan；若目标价变化且资金、AI notional cap 和仓位 cap 允许，会在真正 POST 前更新本地 intent price，否则取消本地 intent 等下一轮重新计划。AI advisory 不再从上一版 quote plan snapshot 搬运，info-risk/AI gate 均以当前 request/input_hash 命中的缓存为准。
- Rewards AI strategy hint 的 `max_condition_notional_usd` 是同 condition 新增 BUY 的硬 cap；最低 rewards size 或 venue 最小名义金额把报价腿放大后，precheck、live placement 和 BUY last-look 都会用放大后的 condition notional 复核，超限时不创建或不提交本地 BUY intent。BUY last-look 即使目标价未变化，也会重新检查资金预算、AI notional cap 和仓位 cap。
- Smart Money Intelligence 重构已开始落地 foundation：`/api/v1/smart-money` 读取 snapshot、`/api/v1/smart-money/config` 保存配置、`/api/v1/smart-money/candidates/status` 更新候选钱包状态（可按 wallet+source 或 wallet 全来源更新，未入库钱包创建 `manual` 来源记录）；数据库已有 smart money config、candidate/profile/score/trade/signal/advisory/paper 表，`SmartMoneyStore` 已接入 `smart_signal_advisories` 缓存读写并在 snapshot DTO 中返回最近 signal advisory，application 已能构造 signal advisory provider payload 和稳定 input_hash；`polyedge-worker scan-smart-money-once`、`poll-smart-money` 和可选 API 内嵌 runtime 定时任务会从 Polymarket Data API leaderboard 和 active copytrade tracked wallets 派生候选，再按 tracked/watch/candidate 状态扫描候选钱包，抓 Polymarket Data API activity/positions/closed positions/trades，保存近端样本画像、确定性评分和 activity 源交易；随后读取 orderbook 服务缓存，为尚未处理的源交易按年龄、盘口、方向价格、滑点和最优档深度生成 `observe` 或 `rejected` 信号，并写入 `stage=deterministic_gate` 的 signal decision 审计记录；开启 `signal_advisory_enabled` 时还会为近期 observe 信号补齐源交易/profile/score 上下文，按 Smart Money 配置中的 `signal_advisory_provider` / `signal_advisory_request_format` / `signal_advisory_model` 构造 advisory payload/input_hash 并检查缓存，provider key/base URL/timeout 来自 `POLYEDGE_SMART_MONEY__SIGNAL_ADVISORY_*` 环境变量，有 key 时按 `signal_advisory_concurrency_enabled` / `signal_advisory_max_concurrency` 调用 `SmartSignalAdvisoryConnector` 保存 `allow|observe|reject` 三态 advisory 并写入 `llm_calls(task_type=smart_signal_advisory)`，无 key 时只记录候选、缓存命中和待请求数量；任一来源已标记 `blocked` 或 `rejected` 的钱包不会被自动 seed 或扫描。`/copy-trading` 已提供 Smart Money 配置保存（含 signal advisory provider/request format/model 和并发设置）、候选池查看、候选状态操作和基础信号流/advisory 展示入口。定时任务默认关闭，需 `POLYEDGE_WORKER__POLL_SMART_MONEY=true` 且 Smart Money config `enabled=true`；若 Smart Money config disabled，worker 跳过外部 discovery/profile 请求，但仍可尝试处理已入库 source trades 的信号、decision 和 advisory refresh。已接入的是 leaderboard 种子发现、基础配置、候选管理、deterministic observe/rejected 信号生成、deterministic decision 审计、signal advisory 缓存层、signal advisory request builder、独立 provider 配置和 provider refresh；recent trades/链上完整 discovery、wallet advisory、纸面模拟和 guarded live execution 尚未实现，不能描述为当前可用能力。
- High Probability Pricing 动态高概率市场定价研究已落地 foundation 并按 2026-07-07 收敛方向演进为 Rewards 做市商的 fair value / base-rate 定价模型 provider：数据库已有 `high_probability_config`、`high_probability_market_outcomes`、`high_probability_samples`、`high_probability_bucket_stats`、`high_probability_backtest_runs`（含 `exit_rule_reports`）、`high_probability_backtest_trades`、`high_probability_observations` 和 `reward_market_fair_values`（0058，可被默认关闭的 Rewards 做市商策略在 shadow/guarded 模式读取）；`HighProbabilityService` 和 Postgres/in-memory store 已支持配置、本地 outcome 标签、rewards candle sample input、observe candidate 查询、样本（含结构化价格路径/流动性/时间/风险研究特征 `HighProbabilityFeatureVector`）、分桶统计、只读 research report、即时 baseline walk-forward backtest report、基础退出规则对比、observe gate、持久化 baseline backtest run/trade、observation 和 fair value 快照；纯 `fair_value.rs` 模型把 bucket stats + 当前盘口 + 风险标签融合成保守 `fair_yes_low/mid/high + confidence + uncertainty_cents + reason_codes + live_eligible`，bucket 解析走 5 维回退链（精确→丢 spread→丢 liquidity→丢 time→仅 price→全局先验），`fair_yes` 优先用 YES token 价命中 bucket 否则用 NO token 价补数，`live_eligible` 是独立 gate 的纯 AND，`input_hash` 用 SHA-256 保证跨进程稳定；observe 与 fair value 共用抽取的 `high_probability_sample_from_observe_candidate` helper 保证同候选同 bucket。API 已提供只读 `/api/v1/high-probability`、`/config`、`/buckets`、`/report`、`/backtests`、`/backtest-runs`、`/backtest-runs/{run_id}/trades` 和 `/fair-values`；前端 `/high-probability` 只读研究页已接入上述 DTO 和中文文案，展示研究配置、样本覆盖、加权胜率/期望、即时 baseline 回测、基础退出规则对比、持久化历史回测 run、所选 run 交易明细、最佳/最差 bucket、bucket stats、fair value 定价表与 observations，不提供写操作或交易控制；`polyedge-worker import-high-probability-outcomes <path>` 可从本地 JSON 文件导入 outcome 标签，`build-high-probability-samples-once [limit]` 会从本地 outcome 标签 + rewards candles 构建 first-touch 样本，`refresh-high-probability-buckets-once` 会从已入库已结算样本计算并替换当前模型版本 bucket stats，`run-high-probability-backtest-once` 会按当前配置运行并持久化 baseline 70/30 walk-forward 回测、退出规则摘要和交易明细，`observe-high-probability-once [limit]` 会读取活跃 rewards 最新 candle 候选 + orderbook 服务缓存并写入 `allow/reject/skip` observations，`refresh-high-probability-fair-values-once [limit]` 会把同批候选 + 盘口融合成 fair value 快照 upsert 到 `reward_market_fair_values`（`fair_value_enabled` 默认 false，关闭时返回零报告、不写入）；API 内嵌 worker runtime 也可通过 `POLYEDGE_WORKER__POLL_HIGH_PROBABILITY_OBSERVE=true` 或 `POLYEDGE_WORKER__POLL_HIGH_PROBABILITY_FAIR_VALUES=true` 按各自 interval（默认 300 秒，runtime 下限 60 秒）自动执行只读 observe 或 fair value refresh，默认关闭。该模块不创建订单、不维护 paper fills、不接 live connector、不拥有 kill switch；fair value 的执行消费由 Rewards 做市商初始实现承接且默认关闭。当前没有 LLM/provider 调用路径；全市场 price-history/outcome 自动 producer、完整执行成本/多阶段退出回测、Phase 4 校准与漂移监控、tier-depth/price-impact 特征尚未实现，不能描述为当前可用能力。
...
- 数据库迁移目前到 `0059_reward_market_maker_decisions.sql`；`packages/backend/init.sql` 是按 0001–0059 合并的空库完整初始化脚本，运行时仍保留 `packages/backend/migrations/` 给 `sqlx` 校验和增量迁移使用。
...
- Rewards live maker 已接入真实 post-only 买单提交、撤单、本系统托管订单成交与计分同步、CLOB open-order 反查、可映射 active rewards BUY 收养/重开、外部 active SELL token 覆盖库存补退出、成交后现金/库存/PnL 更新、sibling leg 撤单和 exit/flatten sell 下单；worker 在 managed order 同步后刷新账户开放买单总 notional 观测，并在新增买单准入时把未归属到本系统 managed order 的外部 BUY notional 从可用资金中保守扣除；confirmed fill 保护期外会刷新 CLOB 余额、资金钱包链上 pUSD 回退和 Data API 完整持仓快照，API 只从数据库读取且不再需要 Polymarket 凭证。外部 SELL 仍不接管为 managed order；非 rewards 市场、无法唯一映射 token 的账户范围外开放订单明细同步或奖励结算对账仍待完成。实盘策略仍应沿用“本系统未成交 maker 买单不硬锁全局 pUSD、成交后才更新现金/库存并撤超额挂单；未知外部 BUY 保守占用可用资金”的资金模型。
- Rewards BalancedMerge 已完成候选/profile 分流、挂单/撤单/last-look 兼容、成交后不退出、merge intent 持久化和显式开启后的 Safe proxy CTF merge 广播；自动执行默认关闭，关闭时 `reward_merge_intents.status=unsupported` 仍只是待处理意图记录。submitted merge receipt 的后续链上确认轮询和 redeem 尚未实现。
...
cargo run -p polyedge-worker -- refresh-high-probability-fair-values-once
```
...
- Polymarket connector 没有 mock mode；未配置真实账户/私钥时，不要开启 Polymarket 私有订单、成交或用户 websocket worker 任务。API 内嵌 `WorkerRuntime` 即使看到这些开关为 true，也会在启动 live Polymarket 常驻任务前检查 `POLYEDGE_POLYMARKET__ACCOUNT_ID`、`POLYEDGE_POLYMARKET__PRIVATE_KEY` 和可选三项 API credential 完整性；配置不完整时只记录一次 warn 并跳过对应 job，CLI/connector 直接调用仍返回精确错误。
...
- `POLYEDGE_POLYMARKET__POLYGON_RPC_URL` 默认 `https://polygon-bor-rpc.publicnode.com`；Rewards worker 用它读取资金钱包链上 pUSD 余额，并在 BalancedMerge 自动执行开启时广播 Safe proxy CTF merge；Funding API 用它广播后端资金钱包 USDC/USDT 入金转账，生产环境可替换为自有或有 SLA 的 Polygon RPC。
...
- 部署模板默认开启 news ingestion 的子系统/worker 开关，默认关闭新闻提升为 events/evidences、rewards、copytrade 和私有对账类 worker；代码默认也关闭 execution drain、paper 对账、Polymarket 私有订单/成交/用户 WS worker，需显式开启才运行；旧 arbitrage radar 开关已移除。
...
- Rewards bot 的 `balanced_merge_enabled=false` 是默认值；开启后前端可编辑 `balanced_merge_max_markets`、`balanced_merge_max_open_orders`、`balanced_merge_min_edge_cents`、`balanced_merge_min_market_score`、`balanced_merge_min_market_liquidity_usd`、`balanced_merge_min_market_volume_24h_usd`、`balanced_merge_max_market_spread_cents`、`balanced_merge_quote_bid_rank` 和 `balanced_merge_max_unpaired_position_usd`。这些配置只作用于 `strategy_profile=balanced_merge`，不改变 standard 策略上限；表格会显示 standard/合并策略标签。
...
- Rewards bot 市场质量硬门槛默认是：`min_market_liquidity_usd=1000`、`min_market_volume_24h_usd=1000`、`min_hours_to_end=48`、`max_market_spread_cents=10`、`max_market_data_age_minutes=15`；`max_market_spread_cents` 同时作为 live token spread gate，实时 `best_ask - best_bid` 超阈值会阻止新 BUY 并撤已有 BUY，SELL 退出不因该规则强制亏损卖出；通过门槛后再按奖励、流动性、成交量、剩余时长和奖励 spread 综合排序。`max_market_data_age_minutes` 同时驱动 orderbook Gamma priority sync 间隔，窗口越小，已注册/活跃/rewards 候选市场刷新越频繁，避免仅因全量 Gamma 目录慢而触发新鲜度撤单。
- Rewards bot 盘口选择默认 `quote_mode=double`、`selection_mode=observe`、`dominant_single_side_enabled=false`，保持 YES/NO 双边计划。启用 auto/enforce 后，planner 阶段只用 `dominant_min_probability` / `dominant_max_probability` 生成初步单边/双边/跳过模式；需要当前盘口的 `dominant_min_exit_depth_usd`、`max_top1_depth_share`、`max_top3_depth_share` 和 `max_book_hhi` 在 live placement materialize 阶段验证。统一机会评分配置包含 `opportunity_metrics_enabled`、probe notional、最低 100U 日奖、最大竞争倍数（软上限，仅 warning）、竞争倍数硬门槛开关 `opportunity_competition_hard_gate_enabled` 与阈值 `opportunity_competition_hard_gate_multiple`（超过即不可挂）、账户/单市场资金占比、退出深度/滑点、坏成交恢复天数、盘口稳定性窗口/样本/波动/跳变阈值和 reward/competition/exit/stability 权重。AI advisory 配置包含 `ai_advisory_enabled`、`ai_provider=openai|anthropic`、`ai_request_format=openai_responses|openai_chat_completions|anthropic_messages`、TTL、`ai_strategy_hint_enabled`、`ai_strategy_hint_min_confidence`、`ai_provider_concurrency_enabled`、`ai_provider_primary_max_concurrency` 和 `ai_provider_fallback_max_concurrency`；信息风险配置包含 `info_risk_enabled`、`info_risk_mode=observe|enforce`、`info_risk_avoid_level=low|medium|high|critical|unknown`、TTL、`require_info_risk_before_first_quote` 和 `first_quote_quarantine_sec`（默认 600）。API key/base URL/model/timeout、advisory allow 最低置信度、info-risk 最低置信度、web search 开关和每轮 provider 上限来自 worker 环境变量，不会进入前端或 API snapshot；`POLYEDGE_REWARDS__INFO_RISK_MAX_MARKETS_PER_CYCLE` 对 full-tick combined provider refresh 表示真实外部调用上限，对 AI advisory 未启用时的独立 info-risk scanner 仍表示候选 condition 裁剪上限。可选第二个完全独立的 LLM 备用接口通过 `POLYEDGE_REWARDS__AI_FALLBACK_*` 五项同时配置启用，主接口任意失败时用同一 combined request 重试备用接口，两次尝试都写入 `llm_calls` 并用 `fallback_used` 区分。
- Rewards bot 本系统未成交 post-only maker 买单不在本地按全局 notional 硬锁资金；不同 condition 可复用同一资金池，但同一 condition 的已有 managed BUY 剩余 notional 与待补 YES/NO 腿必须合计不超过最近同步的 `available_usd` 扣除未归属外部 BUY notional 后的余额，否则整组不挂。full tick 会在 AI/info-risk provider refresh 前对无开放订单/持仓的新 condition 先执行这项最低 rewards size 资金预检查，明显资金不足的计划不再消耗 provider token；已有订单/持仓 condition 跳过前置资金拦截，仍继续进入 provider 风险覆盖。CLOB open-order snapshot 会把可映射 active rewards BUY 收养/重开为 managed order，并且账户级 `external_buy_notional` 只累计 snapshot 中剩余量为正且状态非终态的 BUY；每次 CLOB open-order snapshot 还会用同一次 snapshot 的两侧相减冻结 `unmanaged_external_buy_notional = external_buy_notional - managed_external_buy_notional`（真正的非本系统外部占用），funding precheck 与统一机会评分直接读这个冻结值，而不是用旧的 `external_buy_notional(15s 慢字段) - managed(实时)` 重新相减——避免本系统在两次 snapshot 之间撤掉自己的 managed BUY 时把已撤单 notional 当外部占用、周期性把 `eligible_markets` 压到 0；外部 active SELL token 会覆盖库存补退出但不接管为 managed order，非 rewards 市场和无法唯一映射 token 的外部开放订单明细仍缺失；`stale_book_ms` 默认 45000，配置归一化下限为 5000ms，不再允许生产配置把盘口年龄检查降到 0；worker 会对本地缺失、过期或超过新挂单 freshness headroom 的盘口用 orderbook 服务 HTTP batch 兜底刷新，新挂单要求盘口距离 stale 边界仍有余量；full tick 在 AI/info-risk gate、订单同步和账户同步之后，进入撤单/待提交 intent/新挂单前会对当前 open-like 订单与 eligible quote plan token 再做一次本地 cache / HTTP batch 刷新，避免 tick 前半段 I/O 耗时让初读盘口在 placement 阶段变旧；近期已提交 BUY 的单纯 stale 撤单会短暂 grace，缺盘口/空盘口和其他硬风险不延迟。
...
- Rewards bot 对外部订单 404 会先保持对账锁；若超过 5 分钟仍无 CLOB/Data API 成交证据，则将本地订单标记为 `cancelled`，使其不再计入开放挂单。普通已提交 open-like BUY 若在 CLOB open orders snapshot 中缺失或 snapshot 行已非 active 且无活跃对账锁，只有同轮 managed order 状态/成交对账可靠时才会本地标记为 `cancelled`；若单订单查询失败且 Data API fallback 也没有可信成交证据，本轮只更新 open-order 观测和收养/重开，不用 snapshot 关闭 absent BUY；其中撤单已 accepted 并处于 `cancel accepted; awaiting final reconciliation` 的 BUY，只要后续 open-order snapshot 确认该 external order 已不存在，也会自动关闭本地剩余量并释放新增买单锁，避免单笔已不存在订单长期死锁。提交结果未知订单现在会在恢复查询确认 CLOB 无对应 open 订单后经过 `LIVE_SUBMISSION_UNKNOWN_CLOSE_AFTER_SECS`（默认 600 秒）宽限自动本地关闭（与 404 锁一致），不再永久卡死；取消结果未知订单仍不会仅因本地等待超时 force-cancel。旧 `auto_cancel_stale_minutes` 配置键读取时忽略。
...
- 默认 High Probability observe worker 是 disabled；需要设置 `POLYEDGE_WORKER__POLL_HIGH_PROBABILITY_OBSERVE=true` 后，才会按 `POLYEDGE_WORKER__HIGH_PROBABILITY_OBSERVE_INTERVAL_SECS`（默认 300 秒，runtime 下限 60 秒）持续读取本地候选和 orderbook 服务缓存并写入只读 observations，单轮候选上限复用 `POLYEDGE_WORKER__TASK_LIMIT`。`POLYEDGE_WORKER__POLL_HIGH_PROBABILITY_FAIR_VALUES=true` 同理按 `POLYEDGE_WORKER__HIGH_PROBABILITY_FAIR_VALUE_INTERVAL_SECS` 周期把 bucket stats + 盘口融合成 fair value 快照 upsert 到 `reward_market_fair_values`；fair value provider 受 `HighProbabilityConfig.fair_value_enabled`（默认 false）总开关控制，关闭时返回零报告、不写入。两个循环都不抓外部 Polymarket API、不下单。
...
- Docker 部署中没有单独的 `polyedge-worker` service；`polyedge-api` 只加载 `.env.api` 并在同一进程内启动 worker runtime。部署模板默认启用新闻采集，其他后台循环仍显式设为 `false`；需要运行 rewards、copytrade、新闻提升或私有对账任务时必须在 `deploy/.env.api` 显式设为 `true`，且 live Polymarket 任务还要求 account_id/private_key 配置完整。市场同步和 orderbook 订阅由独立 `polyedge-orderbook` 服务管理，不需要在 worker 中启用。
