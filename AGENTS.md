...
- Rewards BUY last-look 当前按 quote plan token 集合请求 orderbook 服务 1 秒 max-age books，并用刷新 books 重新 materialize 当前 plan；若目标价变化且资金/仓位 cap 允许，会在真正 POST 前更新本地 intent price，否则取消本地 intent 等下一轮重新计划。AI advisory 不再从上一版 quote plan snapshot 搬运，info-risk/AI gate 均以当前 request/input_hash 命中的缓存为准。
