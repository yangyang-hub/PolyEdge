...
- Rewards info-risk provider payload 会携带 `evaluation_time_utc` 和 imminent 判定策略；Chat Completions、Anthropic Messages 和 Responses prompt 都要求按该 UTC 时间判断 `resolution_imminent`，避免 provider 使用训练日期或过期上下文把远期/历史事件误判为临近结算。info-risk cache key `schema_version=6` 会让旧多状态、未携带 provider_cache_policy、旧低竞争策略字段或仍包含 `quote_mode`/`recommended_quote_mode` 的缓存失效并重新评估；`evaluation_time_utc` 不进入稳定 cache key，避免每轮刷新造成缓存雪崩。info-risk 与 AI advisory cache key 都已剔除每轮易变的 materialized `quote_mode`/`recommended_quote_mode`（advisory `schema_version=11`）：这些模式在资金边界市场上 tick 间在 double/single_no 间抖动，留在 key 里会让缓存查不到、把 enforce 模式下的 eligible 计划间歇性打成 pending（最终可挂市场周期性归零）。
