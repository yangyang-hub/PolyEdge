...
- Rewards bot 已是有状态的逐 tick 做市模拟引擎：扫描 Polymarket CLOB rewards 当前市场、从 Redis 盘口缓存读取候选盘口、生成 YES/NO post-only 双边买单计划，并维护共享资金池账本（capital/available/reserved/realized_pnl/reward_earned）。模拟买单会占用 reserved 资金，取消未成交买单会释放占用；缺少新鲜缓存盘口时不会模拟成交或计提奖励；成交模拟只在新鲜盘口穿透/触顶时触发（确定性伪随机可复现）；成交后策略（加价出场 / 持有续挂 / 市价平仓 / 成交即撤对侧）、撤单策略（中点漂移、掉出 max_spread）、以及基于 Polymarket Qmin 公式的做市奖励金额累加已具备；当前仍不会实盘下单。
