...
| `packages/orderbook/src/candle_history.rs` | Rewards candle history sync — 限速调用 CLOB `/prices-history` 写入 5m price-history source candles |
...
| `packages/backend/crates/connectors/src/reward_ai_tests.rs` | Rewards AI advisory connector tests — 二值 `allow_quote` 解析与旧 `suitability` 响应 fail-closed 兼容 |
...
| `packages/backend/apps/worker/src/worker/rewards/provider_refresh_batch.rs` | Rewards AI advisory / info-risk main refresh batch helper — 按配置批量请求、逐项保存、记录实际 provider 调用、漏项/错配回退单市场 |
...
| `packages/backend/apps/worker/src/worker/rewards/provider_refresh.rs` | Rewards AI advisory / info-risk provider refresh — full tick 后分别启动 AI advisory 与 info-risk 后台 task，各自开放订单/持仓优先、各自应用 pre-LLM gate 后按约 2:1 混排普通与低竞争 pre-AI 市场；AI task 使用 `rewards_ai_provider` 临时盘口批次，info-risk task 独立补缓存；实际 provider 调用写入 `llm_calls` |
| `packages/backend/apps/worker/src/worker/rewards/provider_refresh_candidates.rs` | Rewards provider refresh candidate ordering — 开放订单/持仓优先、普通与低竞争 pre-AI 市场混排 |
...
| `packages/backend/crates/application/src/rewards/service_snapshot.rs` | RewardBotService snapshot aggregation — orders/plans pagination、low-competition report and daily LLM usage stats |
...
| `packages/backend/crates/application/src/rewards/provider_prefilter.rs` | Rewards provider pre-LLM hard gate — 保留开放订单/持仓，过滤低竞争 observe shadow 和未通过指标/资格的无敞口计划 |
...
| `packages/backend/crates/application/src/rewards/low_competition.rs` | Rewards low-competition sleeve entry metrics/gate — competition notional、reward/100/day、资金占比、exit depth/slippage、book stability |
| `packages/backend/crates/application/src/rewards/low_competition_cancel.rs` | Rewards low-competition live cancel gate — 宽松撤单阈值、30 秒竞争确认、资金占比/退出/稳定性撤单原因 |
...
| `packages/front/src/features/rewards/components/rewards-tables.tsx` | Rewards tables — quote plan/订单/持仓/成交/事件展示，AI/info-risk 决策展示为“允许挂单/不允许挂单”二值 |
...
| `packages/api/src/handlers/funding.rs` | Funding API — 读取后端资金配置与资金钱包 USDC/USDT 链上余额、幂等校验、step-up 校验并委托 connector 执行 Polymarket Bridge 入金转账 |
...
| `packages/backend/apps/worker/src/worker/rewards/live_pending.rs` | Rewards durable intent submit/recovery workflow — BUY 提交前 last-look，低竞争 BUY 覆盖全计划 token |
...
| `packages/backend/apps/worker/src/worker/rewards/live_placement_limits.rs` | Rewards live placement funding/pre-provider precheck helpers — 同 condition BUY 预算、最低 rewards size 缺口、低竞争开放订单占比 |
| `packages/backend/apps/worker/src/worker/rewards/live_cancel.rs` | Rewards live cancel reason routing — 通用硬风控、低竞争专用撤单分支、普通市场 depth/rank/history/requote 规则 |
...
| `packages/backend/apps/worker/src/worker/rewards/event_cancel.rs` | Rewards orderbook event-driven hard-risk cancel worker — 独立消费活跃 token 更新并立即 cancel-only 撤单；当前低竞争 plan 的订单会在自身或 companion token 更新时补齐同计划盘口并只跑低竞争专用撤单 gate |
...
| `packages/backend/init.sql` | Complete empty-database initialization script generated from migrations 0001–0048 |
...
- 前端控制台已有 `dashboard / markets / events / rewards / funding / copy-trading / wallet-analysis / settings` 页面；`radar / signals / positions / risk` 旧入口已移除，`/replay` 和未落地的 approvals 页面不再作为前端入口暴露。
- 前端控制台导航在桌面端使用左侧折叠 sidebar，在移动端通过顶栏菜单按钮打开左侧抽屉；两者共享同一份导航项配置，并使用原生链接跳转以适配静态导出部署。
- 前端数据层统一走 `src/lib/api/*`（读取按领域文件 `markets.ts` / `rewards.ts` / `funding.ts`… 基于 `base.ts`，写操作通过 `actions.ts` barrel 暴露、实现按领域拆在 `actions/`），页面装配在 `src/features/*/loaders` 和 `src/features/*/components`。`src/server/` 目前是空目录（历史遗留）。
...
- 后端 API 已覆盖 markets、events、news、evidences、orders、trades、pricing、rewards bot、funding、copytrade、wallet analysis、system、connector callback 和 orderbook（`GET /api/v1/orderbook/{token_id}`）等主路径；`/api/v1/signals`、`/api/v1/positions`、`/api/v1/risk/*` 和 `/api/v1/arbitrage/*` 旧控制台 API 已移除。
...
- Rewards 低竞争市场 sleeve v2 已实现并默认关闭（见 [doc/rewards-low-competition-sleeve-plan.md](./doc/rewards-low-competition-sleeve-plan.md)）：使用独立 `standard` / `low_competition` candidate profile，不全局降低主策略流动性/成交量硬门槛；低竞争候选不使用 liquidity/24h volume 作为准入门槛，后端保留旧过滤字段用于兼容但归一化强制关闭并置 0，前端不再暴露这些控件且保存配置时会强制关闭并清零；低竞争候选按竞争程度/奖励密度而非标准流动性排序，并可用低竞争专属 `quote_bid_rank`（默认买二）、安全边际（默认 2c）、rewards spread（默认 6c）、Gamma market spread（默认 8c）和最低 market score（默认 8）覆盖主策略报价/筛选参数；为区分“伪低竞争”（流动性低但盘口竞争极激烈），拿到真实盘口后会按 `low_competition_candidate_max_competition_multiple`（默认 5）给 competition_multiple 超标候选打 `not_low_competition` 早期剔除标签，只用于 prewarm/observation 降级和 shadow report 的“高竞争混入占比”统计，不进入正式 gate 的 rejection_reasons、不影响 `eligible_for_low_competition` 或 enforce；`observe` 只写入 `strategy_bucket`、低竞争指标和 observation，不实盘下单，也不会为无开放订单/持仓的 observe-only shadow 计划触发 AI/info-risk provider 调用；`enforce` 默认用 `low_competition_probe_notional_usd=10` 计算 competition share / competition multiple，要求竞争份额、账户与单 condition 挂单资金占比、预估 reward/100/day（默认至少 0.50）、退出深度、入场退出滑点、坏成交恢复天数、盘口历史样本、midpoint 稳定性和 top-of-book 跳变全部达标，并要求 AI advisory 开启且 info-risk 为 enforce；低竞争可要求高置信度 AI `allow`（默认开启）和更严格的 info-risk avoid level（默认 medium），同一 condition 的低竞争候选会替换 standard 候选，之后仍由既有 AI/info-risk cache gate、live materializer、kill switch、订单/库存/账户外部 BUY notional 风控 fail closed；低竞争 gate 前会先用 `rewards_low_competition_probe` source 按最多 10 个市场一批预热盘口和历史样本；通过低竞争 gate 和 pre-LLM provider gate 的低竞争 pre-AI 市场会与普通 eligible/candidate 市场按约 2:1 混排进入 AI advisory refresh 的每批 10 个市场临时盘口订阅和 AI 分析，info-risk refresh 使用独立队列补缓存。缺 fresh midpoint、盘口指标或历史样本时会保留 quote legs 和 `orderbook_token_ids` 并标记 data unavailable。低竞争 managed order 会持久化 `strategy_bucket=low_competition`；worker 会用 gate 前固化的计划 notional、competition share 和挂单资金占比持久化 `reward_low_competition_observations`，API snapshot 返回最近 24 小时 shadow report（含坏成交恢复天数 P95）和保守小额 enforce 建议，但不会自动切换配置；前端 quote plan 行内展示硬性通过/未过状态，并按竞争、奖励、定价/事件风险、退出能力、盘口稳定性和仓位逐项显示当前值、门槛与通过状态，策略观察面板显示低竞争策略状态、硬性通过率、AI/信息风险后可挂率和主要卡点；live placement 会按 `low_competition_global_open_order_share_bps` 把低竞争 open-like 订单限制在全局 `max_open_orders` 的默认 30% 内（全局允许时至少 1 单），低竞争待提交 BUY 在 POST 前会对同计划 `orderbook_token_ids` 和报价腿 token 做 1 秒 max-age last-look，缺任意必要盘口即 defer；挂单后常规撤单、待提交/last-look 和事件驱动 hard-risk 撤单都会复核低竞争 gate，事件路径会在订单自身或 companion token 更新时补齐同计划盘口，竞争度或资金占比硬失败会撤单。
...
- Rewards live 会在提交旧 intent 前先执行当前盘口/资格撤单检查；BUY intent 在真正 POST 前还会用 orderbook 服务做 1 秒 max-age last-look，若盘口缺失、刷新失败或出现 best ask touch 等硬风险则不提交；普通 BUY 只要求提交 token，低竞争 BUY 要求同计划 `orderbook_token_ids` 和报价腿 token 全部返回 1 秒 max-age 盘口，缺任意 token 即 defer。任一提交结果未知、待最终对账或外部订单 404 会暂停全部新增买单，但继续同步、撤单和卖出退出；外部订单 404 锁超过 5 分钟且仍无成交证据时会自动本地关闭。提交结果未知时，开放订单严格匹配失败会先保持对账锁；当恢复查询确认 CLOB 无对应 open 订单并经过 `LIVE_SUBMISSION_UNKNOWN_CLOSE_AFTER_SECS`（默认 600 秒）宽限后，worker 会把该订单本地标记为 cancelled 以释放锁（与 404 锁一致），但若 positions 快照显示该 BUY token 在提交后出现库存则继续保留对账锁。CLOB `post_order` 只要返回订单 ID 就保留为 accepted 供后续成交/状态对账，包含 `unmatched` / `canceled` / 未知状态；HTTP 4xx 明确拒单会标记当前 intent 为 error，只有网络中断、5xx 或成功响应缺少订单 ID 才进入提交结果未知锁。managed order 的后续 upsert 会同步更新实际提交数量；SELL intent price 保留非亏损退出 floor；post-only exit 被取消后的重试保留退出 floor 并按 maker 规则重试，flatten replacement 保留退出 floor 并在后续按 best bid 非亏损 FAK 或继续等待。订单 scoring 观测只推进 `last_scored_at`，不修改业务状态 `updated_at`；reconciliation 锁订单跳过 scoring 查询，避免周期性观测掩盖真实业务状态年龄。
...
- 数据库迁移目前到 `0048_reward_account_unmanaged_buy_notional.sql`；`packages/backend/init.sql` 是按 0001–0048 合并的空库完整初始化脚本，运行时仍保留 `packages/backend/migrations/` 给 `sqlx` 校验和增量迁移使用。
...
- Rewards 低竞争市场 sleeve 已有 observe/enforce v2、独立小额度配置、竞争/奖励/退出/稳定性/坏成交恢复/provider 指标 gate、跨周期 observation 和策略观察面板；低竞争候选强制不使用 liquidity/24h volume 作为准入门槛，旧低竞争 liquidity/volume 过滤字段兼容读取但归一化后关闭并置 0；前端 quote plan 行内按竞争、奖励、定价/事件风险、退出能力、盘口稳定性和仓位分组展示低竞争硬性通过状态和逐项门槛；低竞争待提交 BUY 的 last-look 会强刷并要求全计划 token，挂单后常规撤单和事件驱动 hard-risk 撤单会补齐必要盘口并走低竞争专用可配置撤单 gate，不再复用普通市场 depth/rank/history/requote 撤单规则。仍缺自动启用/自动切换 enforce。
...
- 活跃 token 盘口事件会先进入独立 `RewardEventCancelGuard` hard-risk cancel worker：普通订单按更新 token 检查开放订单，低竞争订单在自身或 companion token 更新时补齐当前 plan 全量盘口后复查；事件路径会立即处理缺盘口/空盘口、SELL stale、计划失效、kill switch、post-only violation retry 等硬风险，普通 BUY 还会检查深度/rank/盘口历史窗口，低竞争 BUY 只跑低竞争专用 gate；不做成交同步、账户同步、退出提交、重挂、报价漂移换价或定期 requote。
