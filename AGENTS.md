...
| `packages/backend/apps/worker/src/worker/rewards/event_cancel.rs` | Rewards orderbook event-driven hard-risk cancel worker — 独立消费活跃 token 更新并立即 cancel-only 撤单；低竞争订单补齐同计划 companion token 后只跑低竞争专用撤单 gate |
...
- Rewards 低竞争市场 sleeve 已有 observe/enforce v2、独立小额度配置、指标 gate、跨周期 observation 和 shadow report；低竞争候选强制不使用 liquidity/24h volume 作为准入门槛，旧低竞争 liquidity/volume 过滤字段兼容读取但归一化后关闭并置 0；挂单后常规撤单、待提交/last-look 和事件驱动 hard-risk 撤单都会补齐必要盘口并走低竞争专用撤单 gate，不再复用普通市场 depth/rank/history/requote 撤单规则。仍缺自动启用/自动切换 enforce。
...
- 活跃 token 盘口事件会先进入独立 `RewardEventCancelGuard` hard-risk cancel worker：只按更新 token 检查开放订单，立即撤缺盘口/空盘口、SELL stale、计划失效、深度/rank/盘口历史窗口、kill switch、post-only violation retry 等硬风险；不做成交同步、账户同步、退出提交、重挂、报价漂移换价或定期 requote。
