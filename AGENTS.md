...
| Reward midpoint candles | `polyedge-orderbook` 服务 | 内部 orderbook cache updates（best bid/ask midpoint） | `reward_market_candles` table (Postgres) | on accepted WS/poll/ingest cache update, 5m buckets |
...
市场和奖励市场由 orderbook 服务同步写入 Postgres，盘口数据由 orderbook 服务流式写入进程内缓存；rewards token 的 5 分钟 midpoint K 线由 orderbook 服务从已接受的内部盘口更新派生并写入 `reward_market_candles`，不包含真实成交量。所有消费者从数据库或 orderbook 服务读取，不直接调用外部 API。
...
| `packages/orderbook/src/updates.rs` | Orderbook update broadcaster — 为 WS/poll/ingest 缓存更新分配 sequence、推送内部 WS，并异步记录 rewards midpoint candles |
...
- `POLYEDGE_ORDERBOOK_STREAM__MAX_TOKENS` 默认 3000；调高会增加 orderbook WS/poll 内存占用，调低会减少 rewards 盘口覆盖。每个 source（活跃 rewards、execution、当前 eligible rewards、其余候选 token）独立注册全量 token，由聚合层按固定优先级跨 source 去重并 take 上限截断；`rewards_eligible` 由周期任务注册全部 eligible quote plan token（不再因 active 持仓覆盖而被清空）。`POLYEDGE_ORDERBOOK_STREAM__REWARD_CANDIDATE_TOKEN_CAP` 默认 100，只限制 rewards 候选预热 source，不影响活跃订单、持仓、execution 或 eligible quote plan token；设为 0 可关闭候选预热以快速降带宽，但 eligible 市场仍会按需订阅。`POLYEDGE_ORDERBOOK_STREAM__WS_CHUNK_SIZE` 默认 250，用于控制每条 Polymarket WS 连接承载的 token 数；调低会减少单连接消息压力、增加连接数，调高则相反。`POLYEDGE_ORDERBOOK_STREAM__MAX_LEVELS_PER_SIDE` 默认 100，用于限制进程内缓存和 HTTP ingest 每个 token 的 bids/asks 保留深度；写入时先排序再裁剪，保留最优档位。poll 每周期会刷新全部注册 token；`POLYEDGE_ORDERBOOK_STREAM__STALE_THRESHOLD_MS=0` 只关闭年龄 stale 优先级。
