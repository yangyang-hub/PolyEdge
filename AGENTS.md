...
- Rewards 成交对账除 404 fallback 外，也会在关联 trade 按 ID 查询失败时按 token/time 扫描认证账户 trades 并按 external order id 精确匹配；任何单笔订单的单查和回退失败都只隔离当前订单，不再阻断其余订单对账、账户持仓同步或 stale 清理。
