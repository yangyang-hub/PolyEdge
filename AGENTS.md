...
| `packages/backend/crates/connectors/src/reward_provider.rs` | Rewards combined provider connector — 单市场一次 LLM 调用可同时返回 AI advisory 与 info-risk，支持 OpenAI Responses、OpenAI-compatible Chat Completions（含 GLM/DeepSeek 模型名特例）、Anthropic Messages 和可选 OpenAI Responses web search |
