...
| `packages/backend/apps/worker/src/worker/rewards/provider_refresh_orderbook.rs` | Rewards provider refresh temporary orderbook source helper — 临时订阅 AI 所需盘口、返回与本轮基础 books 合并后的刷新盘口、allow 后提升 eligible source |
| `packages/backend/apps/worker/src/worker/rewards/provider_refresh.rs` | Rewards combined provider refresh — full tick 应用已有 provider cache gate 后启动单个后台 task，按统一候选顺序和 Rewards 配置的主/备 provider 并发限制补齐到期 AI advisory/info-risk section；使用 `rewards_ai_provider` 按最多 10 个市场一批准备 AI 临时盘口 source，纯 info-risk 批次不等待盘口，并用刷新合并后的 books 构造/校验 advisory 请求；实际 provider 调用写入 `llm_calls(task_type=reward_provider)` |
...
| `packages/backend/apps/worker/src/worker/rewards/live_pending.rs` | Rewards durable intent submit/recovery workflow — BUY 提交前 last-look 刷新当前 plan token 集合、重新 materialize 并在 POST 前重定价或取消本地 intent |
...
| `packages/backend/apps/worker/src/worker/rewards/live_cancel.rs` | Rewards live cancel reason routing — 通用硬风控、depth/rank/history/requote 规则和 strategy profile plan 匹配 |
