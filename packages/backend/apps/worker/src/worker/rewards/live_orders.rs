...
const INTERNAL_ORDER_ID_PREFIXES: &[&str] = &["rew_", "rewx_", "rewfill_", "rewevt_"];

/// Returns `true` when `id` was generated internally (prefixes like `rew_`,
/// `rewx_`) rather than submitted to Polymarket (which returns plain UUIDs).
/// Guarding on this prevents sending fake IDs to the CLOB cancel or
/// trade-sync APIs.
