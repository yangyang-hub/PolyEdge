...
impl DatabaseMaintenanceStore for PostgresDatabaseMaintenanceStore {
    async fn prune_database_history(
        ...
        let reward_risk_events_deleted = execute_maintenance_delete(
            &self.pool,
            "reward_risk_events",
            r#"
            WITH doomed AS (
            ...
            DELETE FROM reward_risk_events rows
            USING doomed
            WHERE rows.ctid = doomed.ctid
            "#,
            cutoffs.reward_risk_events_before,
        )
        .await?;
