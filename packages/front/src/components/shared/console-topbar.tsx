...
import { humanizeSnakeCase } from "@/lib/realtime-formatters";
...
export function ConsoleTopbar() {
  ...
  const modeLabel = lastEvent?.data.mode ? humanizeSnakeCase(lastEvent.data.mode) : "runtime sync";
  const environmentLabel = lastEvent?.data.environment ?? "stream sync";
      ...
      <div className="flex h-14 items-center justify-between gap-6 px-4 md:px-6">
        <div className="flex min-w-0 flex-1 items-center gap-6">
          ...
          <div className="relative hidden w-full max-w-sm md:block">
            ...
            <Input
              ...
              placeholder="Search markets, signals..."
            />
          </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <StatusPill tone={warningCount && warningCount > 0 ? "warning" : "neutral"}>
            {warningCount !== undefined ? `${warningCount} warning` : "risk sync"}
          </StatusPill>
          <StatusPill tone="neutral" className="hidden md:inline-flex">
            {criticalCount !== undefined ? `${criticalCount} critical` : "alerts sync"}
          </StatusPill>
          <Button
            asChild
            size="sm"
          ...
          <div className="hidden items-center gap-3 border-l border-white/8 pl-4 md:flex">
            <div className="text-right">
              <p className="text-xs font-semibold uppercase tracking-wide text-foreground">J. Trader</p>
