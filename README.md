...
- 前端控制台已经有 `dashboard / markets / events / radar / rewards / signals / positions / risk / approvals / replay / settings`
- 后端已经有 Axum API、worker 子命令、配置和数据库迁移
- 前端已经移除 mock 数据模式，读写和 SSE 都要求连接真实 Rust 后端
- 本地 API/SSE 联调路径已经基本打通，生产化链路仍未闭环
...
控制台 `/rewards` 已接入 Polymarket CLOB rewards 当前市场扫描和模拟报价计划。当前实现只生成保守的 YES / NO post-only 买单计划和模拟托管挂单，不会实盘提交订单。
...
1. `/rewards` 页面也可以通过后端 API 手动运行一次模拟；静态前端会通过 Nginx 同源 `/api/v1` 代理访问 Rust 后端。
2. `mode=live` 目前不会实盘下单，后端会记录告警并继续按模拟链路生成计划。
3. 配置 PostgreSQL 后，API 和 worker 可共享 rewards 市场、报价计划、模拟挂单和事件；不配置 PostgreSQL 时仅当前进程内存可见。
...
- `dashboard / markets / events / radar / rewards / signals / positions / risk / approvals / replay`
- feature loader + feature component 分层
- Server Actions 驱动的审批、风险控制、rewards bot 模拟和 kill switch UI 链路
...
6. Rewards bot 当前只闭合到模拟报价和模拟托管挂单，尚未接入实盘下单、订单计分查询、成交后退出单或真实库存处理。
7. 新闻源当前已闭合到 `raw_events` 入库、健康状态、只读查看和保守提升为 `events/evidences`，尚未自动生成 `signals`。
