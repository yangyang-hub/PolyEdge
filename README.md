...
本地 `cargo run` 的 server 默认可通过 `http://127.0.0.1:38001` 访问，前端 Docker 位于 `http://127.0.0.1:33002`。Compose 默认只把 server 发布到宿主 `127.0.0.1:38001`；前端必须保持 `NEXT_PUBLIC_POLYEDGE_API_BASE_URL=`，通过 Nginx 同源 `/api` 代理访问同机或远程内网 server。
...
前后端拆机时，后端机在 `.env.server` 设置 `POLYEDGE_SERVER_PUBLISH_BIND=<后端内网 IP>`，前端机在 `.env.front` 设置 `POLYEDGE_FRONT_API_UPSTREAM=http://<后端内网 IP>:38001`。两台机器分别运行 `./scripts/deploy.sh server` 与 `./scripts/deploy.sh front`；若使用周期 `auto`，再分别设置 `POLYEDGE_SKIP_SERVICES=front|server`。浏览器仍只访问前端地址。HTTP-only 内网必须使用 server `local` runtime、将 `PUBLIC_ORIGIN` 精确设为前端 origin，并保持 CORS 为空；后端 38001 应由防火墙限制为仅前端主机可访问。
