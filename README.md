...
- 后端已有 Axum API、worker 子命令、独立 orderbook 服务、配置和数据库迁移（目前到 `0023`）
...
1. 前端发布产物是静态文件，运行时由 Nginx 容器托管，并通过同源 `/api/v1/...` 反向代理到 Rust 后端。
2. 受保护写操作仍会走 step-up code；静态前端代理会把校验结果转成后端 local dev-auth 所需 header。
...
- `nginx.conf.template`：静态资源和 `/api/v1` 后端代理配置
...
1. Docker 部署默认不需要配置前端 API base URL；Nginx 会把同源 `/api/v1` 代理到 `POLYEDGE_API_UPSTREAM`。
2. `NEXT_PUBLIC_POLYEDGE_API_BASE_URL` 只用于本地浏览器绕过 Nginx 直接请求 Rust API 的场景。
...
控制台 `/rewards` 已接入 Polymarket CLOB rewards 当前市场扫描和 live 实盘报价。Rewards bot 仅支持 `live` 模式，通过 `OrderbookHttpClient`（HTTP 调用 polyedge-orderbook 服务）读取盘口，worker 通过 `LivePolymarketConnector` 提交 post-only GTC token 买单，成交后按配置撤 sibling legs 并执行 exit-at-markup 或 flatten sell。
...
4. 独立账户余额/库存全量对账、订单计分查询和奖励结算对账仍是缺口。

### 聪明钱跟单（Copy-trading）

控制台 `/copy-trading` 已接入完整的模拟跟单子系统：跟踪多个 Polymarket 钱包地址、通过 Polymarket Data API 检测新成交、四种跟单仓位模式（`FixedUsd` / `ProportionalToSource` / `CapitalRatio` / `MirrorPortfolioWeight`）、钱包分析统计、per-wallet/per-market/total 风控和确定性模拟引擎。
...
部署脚本默认使用 `/tmp/polyedge-deploy.lock` 防止 cron/CI 重叠执行，默认 `COMPOSE_PARALLEL_LIMIT=1` 串行构建镜像。Auto 模式只有后端二进制（api / worker / orderbook）或前端文件 hash 改变时才 rebuild。Compose 构建上下文已收窄：后端只上传 `bin/`，前端只上传 `packages/front/`，避免扫描本地 `packages/backend/target`、`node_modules`、`.next` 等大目录。

Docker 部署里 worker 后台任务默认由 Compose 覆盖为关闭状态；需要新闻、套利、rewards 或 copytrade 时，在 `deploy/.env.worker` 显式设置对应 `POLYEDGE_WORKER__...=true`。市场同步和 orderbook 订阅由独立 `polyedge-orderbook` 服务管理，不需要在 worker 中启用。
...
所有外部 API 数据由后台 worker 获取并写入数据库或内存缓存，策略、页面和 API handler 只从这些 store 读取，不直接调用外部 API。
...
- Rewards bot live 实盘：post-only GTC 下单、撤单、成交同步、cash/库存/PnL 更新、sibling leg 撤单和 exit/flatten sell
- 聪明钱跟单：四种仓位模式、钱包分析统计、per-wallet/per-market/total 风控、确定性模拟引擎、控制命令队列
- Polymarket CLOB V2 connector，支持 `eoa` / `proxy` / `gnosis_safe` / `poly_1271` 签名
...
4. Rewards live maker 已接入真实下单和成交同步；仍未完成独立账户余额/库存全量对账、订单计分查询或奖励结算对账。
5. 聪明钱跟单 `mode=live` 已结构化支持但未接入真实下单。
6. Polymarket live 链路已具备 CLOB V2 SDK 和认证能力；仍未实现 Deposit Wallet 生命周期管理，且仍需真实资金链路小额验证。
7. 默认部署模板仍沿用本地 internal dev-auth 模式，只适合原型/内网共享环境。
