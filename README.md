...
1. 前端必须配置 `POLYEDGE_API_BASE_URL`，读数据会请求后端 `/api/v1/...`，SSE route 会代理 Rust `/api/v1/stream/{channel}`。
2. 受保护写操作仍会走 step-up code 与 internal API auth。
...
- `src/app/api/stream/[channel]/route.ts`：SSE proxy 入口
...
1. `POLYEDGE_API_BASE_URL` 必须指向真实后端；前端不再提供 mock 数据模式。
2. `POLYEDGE_CONSOLE_AUTH` 当前只保留 `off`；生产级 session provider 尚未接入。
...
1. `/rewards` 页面也可以通过后端 API 手动运行一次模拟；前端仍要求 `POLYEDGE_API_BASE_URL` 指向 Rust 后端。
