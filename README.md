...
- [V3 原始设计](./doc/designs/manual-market-maker-v3.md)（仅作单用户执行语义背景，身份与多租户状态以当前文档为准）
...
- 策略版本固化 condition、YES/NO token、rewards 条款和 quote slots，并只在 `[active_from, active_until)` 内做市。
...
默认地址：server `http://127.0.0.1:38001`，前端 Docker `http://127.0.0.1:33002`。生产前端应保持 `NEXT_PUBLIC_POLYEDGE_API_BASE_URL=`，通过 Nginx 同源 `/api` 代理访问 server。
...
- 管理员用户页尚无禁用/角色编辑 UI；following 页尚无 discover 选择器和暂停/停止编辑 UI。
- 前端危险操作仍显示旧 step-up code 输入框，尚未接到 `/auth/reauth`；后端只接受 recent-auth session。
