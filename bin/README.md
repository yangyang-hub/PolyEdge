...
V4 deployment uses one Rust artifact only:
...
Legacy `polyedge-api`, `polyedge-orderbook`, `polyedge-worker`, and `polyedge-replay` binaries are not V4 deployment artifacts and must not be referenced by Docker Compose or deployment scripts.
