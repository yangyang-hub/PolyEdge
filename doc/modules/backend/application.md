...
- AI advisory 定义低频模型判断的输入、输出和执行约束：`build_reward_ai_advisory_request()` 从开放订单、持仓和已通过初步 planner 筛选的 eligible 奖励市场、确定性计划、账户/仓位/开放订单、盘口 top levels、最近最多 24 小时的 5m price-history source candles 聚合出最多 24 根 1h candles 和 candle summary 构建完整 provider payload；payload 还显式携带 `pricing_context`（当前 best bid/ask、midpoint、binary midpoint sum、报价边际、是否触碰 ask、盘口确认年龄和 spread 合理性摘要）与 `provider_cache_policy`（TTL、正向 jitter 窗口、提前刷新窗口和最晚缓存复用时间），要求 provider 对整个 TTL horizon 只输出 `allow_quote=true|false` 二值决策。`input_hash` 不再使用完整动态 payload，而是用市场身份/问题、奖励参数、计划 quote mode、相关策略配置和已完成小时级 candle summary 组成的稳定 cache-key payload 计算 SHA-256，避免账户余额、开放订单、盘口时间戳、盘口档位或当前小时内 5m source 更新导致后台刚保存的 advisory 下一轮无法命中，同时让已完成小时级趋势/波动摘要变化能触发重新评估；低竞争专属报价、spreads、净收益、provider 加严和撤单阈值也进入低竞争相关 cache-key payload，配置变化会触发重新评估。新保存的 provider cache 会按 condition/provider/request_format/model/input hash 计算确定性正向 TTL jitter（最多 TTL 的 20%，且最多 15 分钟），避免同批写入的 AI/info-risk 记录在同一秒集中失效；同一窗口也作为后台提前刷新阈值，缓存仍有效时 live gate 继续使用旧记录。`apply_existing_reward_ai_advisories()` 只把已有 advisory 挂到 quote plan，不把缺失 advisory 的计划改为 pending；`apply_reward_ai_advisories()` 用于 live tick 的缓存 gate，并在 `ai_advisory_enabled=true` 时只把缺少 advisory 或二值不允许（内部兼容映射为 `avoid`）的原 eligible 计划改为不可挂；二值允许结果会保留 deterministic 报价腿并继续进入 live 盘口/资金风控；旧 `suitability` 响应按二值 fail-closed 兼容，仅 `allow` 视为允许，`watch`/`avoid`/其它非 allow 值一律映射为 `avoid` 硬拦（旧 watch 放行行为已移除），且 live gate `reward_ai_advisory_blocks_quote` 直接把任何非 allow 决策（含从上一版 quote plan 继承、缓存键尚未刷新的旧 watch advisory）判为不可挂，避免旧 watch 通过 plan 继承在 advisory TTL 内继续放行；低竞争 enforce 且 `low_competition_require_ai_allow=true` 时只接受达到置信度阈值的允许结果。历史 `allow` 的 `single_yes/single_no` 收窄只在置信度达到阈值且 `selection_mode=enforce` + `quote_mode=auto` 时应用；新二值 provider 不再输出 quote mode 或 exit policy。后台 AI advisory provider refresh 只写入 `reward_market_advisories` 缓存，不阻塞 live tick，也不使用旧 cycle 覆盖 quote plan snapshot；AI 请求按 worker 设置的每轮 condition cap 扫描，并在请求前应用 `provider_prefilter`：无 active exposure 的低竞争 observe 计划不请求、低竞争 enforce 需先通过指标、标准计划需 eligible/pre-AI eligible；通过 gate 后的无敞口候选会按 `standard` / `low_competition` 类型分桶再混排，低竞争失败不会作为普通候选继续请求模型。AI 请求按最多 10 个市场一批注册 `rewards_ai_provider` 临时 orderbook source，下一批会取消上一批，结束后清空；AI 请求可由配置 `ai_advisory_batch_size` 控制单次 provider 请求包含的市场数（默认 1 保持逐市场，最大 12），批量响应按 condition 拆分保存，漏项/错配市场回退到单市场请求；允许挂单 advisory 后会把该市场 token 即时合并到 `rewards_eligible` source，后续 full tick 再用持久 quote plan 校正。遇到 HTTP 传输失败、429、认证失败或常见 5xx 会停止本轮请求。`reward_market_books_available()` 在后台 refresh 请求前校验该市场所有报价 token 盘口都已发布（非空 bids 与 asks），盘口缺失/为空的市场本轮跳过请求且不写缓存，等 orderbook 订阅/缓存返回盘口后再评估，避免缓存键不含即时盘口的设计下被一条“无盘口不允许挂单”结果卡住整个 TTL；AI advisory 与 info-risk provider refresh 分别使用独立候选队列、独立 running flag 和独立 provider permit，info-risk 不再挤占 AI advisory 的 selected condition 名额。advisory cache key `schema_version` 已升到 8，使旧 watch 放行时期缓存的 advisory 失效并按 fail-closed 二值兼容重新评估（schema_version 7 此前已让旧多状态 advisory 失效）。`reward_ai_advisories_from_quote_plans()` 用于 full tick 新计划保存前从上一版 snapshot 搬运未过期且 provider/request_format/model 匹配的 advisory。默认 `ai_advisory_enabled=false`；配置 wire value 使用 `ai_provider=openai|anthropic` 和 `ai_request_format=openai_responses|openai_chat_completions|anthropic_messages`，后端兼容读取旧 `open_ai*` 拼写但序列化始终输出无下划线 `openai*`。provider confidence 在 connector 解析时钳制到 `0..=1`；不允许挂单仍不能绕过市场质量、盘口和风控硬过滤；成交后退出策略由 `RewardBotConfig.post_fill_strategy` 决定，不再由 advisory `exit_policy` 覆盖。
- Info risk 定义异步信息流风险判断的输入、输出和执行约束：`build_reward_info_risk_assessment_request()` 从奖励市场、当前 quote plan、账户/仓位/开放订单和策略配置构建完整 provider payload 与搜索 query，并在 provider payload 中加入 `evaluation_time_utc`、imminent 判定策略和 `provider_cache_policy`，要求模型按该 UTC 时间和完整 TTL horizon 判断是否允许挂单；新 provider 输出只允许 `allow_quote=true|false` 二值结果，connector 会把允许映射为内部 `low/none`、不允许映射为内部 `critical/unknown` 以兼容既有表结构和 DTO。`query_hash` 仍按 query 计算，`input_hash` 改为用搜索 query、市场身份/问题/事件、计划 quote mode 和风险策略配置组成的稳定 cache-key payload 计算 SHA-256，不包含每轮变动的账户、开放订单、持仓、quote plan reason/score、`market_synced_at` 或 `evaluation_time_utc`。info-risk cache key `schema_version=4` 会让旧多状态缓存失效并重新评估；新保存的 info-risk 记录也使用与 AI advisory 相同的确定性 TTL jitter / 提前刷新窗口。`apply_reward_info_risks()` 会把最新未过期风险挂到 quote plan，低竞争 plan 可用 `low_competition_info_risk_avoid_level` 覆盖主策略的 avoid level，默认更严格到 `medium`。默认 `info_risk_enabled=false`；provider confidence 在 connector 解析时钳制到 `0..=1`。主 info-risk provider refresh 的请求可由配置 `info_risk_batch_size` 控制单次请求包含的市场数（默认 1，最大 12），批量响应按 condition 拆分保存，漏项/错配市场回退到单市场请求；该 refresh 与 AI advisory refresh 独立运行、独立 cap、独立 provider permit，并和独立 info-risk worker 共用 `provider_prefilter`，不会再对没有 active exposure 且没有通过 quote-plan pre-LLM gate 的 market-only 候选发请求。当 `info_risk_mode=enforce` 时，缺少未过期风险缓存会 fail closed，eligible 计划会被置为不可挂；`require_info_risk_before_first_quote`（默认 true）还会要求新 condition 首次 BUY 报价前已有未过期风险缓存，`first_quote_quarantine_sec`（默认 600，0 可关闭）要求新 condition 至少观察对应秒数；已有 open-like 订单或持仓的 condition 跳过该首单 gate。二值不允许结果会硬拦截，不再因为低 confidence 变成第三种“提示”状态；历史 taxonomy 缓存仍按兼容逻辑处理：`critical`、官方结果、`resolution_imminent=true` 或配置为 `low/medium` 避免等级且达到置信度阈值时会拦截，普通 `high` 风险以及仅 `risk_type=imminent_resolution` 但 `resolution_imminent=false` 的结果保留为 quote plan 上的信息提示并继续进入 live 盘口/资金风控。独立 info-risk worker 与 full-tick info-risk provider refresh 都受 `POLYEDGE_REWARDS__INFO_RISK_MAX_MARKETS_PER_CYCLE` 每轮 condition cap 约束（默认 50）。该结果不能绕过市场质量、盘口和风控硬过滤。
...
- 低竞争 rewards sleeve 已实现 v2，默认 `low_competition_mode=off`。它使用独立 `standard` / `low_competition` candidate profile，不全局降低主策略 `min_market_liquidity_usd` / `min_market_volume_24h_usd`；后端配置归一化会强制关闭旧 `low_competition_candidate_*_filter_enabled` 开关并把低竞争 liquidity/24h volume 阈值置 0，因此低竞争 profile 不以 Gamma liquidity 或 24h volume 作为准入门槛，而是共享 open/tradable、高歧义、唯一 YES/NO、结算时间、Gamma spread、数据新鲜度和 FDV/launch/token/official-result 事件风险硬过滤，并在候选查询中按低竞争奖励密度/低流动性优先排序。低竞争可配置专属 `quote_bid_rank`、安全边际、rewards spread、Gamma market spread 和最低 market score，默认使用买二、2c 安全边际、6c rewards spread、8c market spread 和 8 分最低评分。为识别流动性低但盘口竞争极激烈的“伪低竞争”候选（候选 SQL 阶段 markets 表无盘口深度，无法预筛），`build_low_competition_metrics` 会在拿到真实盘口后按 `low_competition_candidate_max_competition_multiple`（默认 5，归一化时不低于正式 gate 阈值）给 competition_multiple 超标的候选打 `not_low_competition` 分类标签，只用于下游 prewarm/observation 降级和 shadow report 的“高竞争混入占比”统计，不进入正式 gate 的 rejection_reasons，不影响 `eligible_for_low_competition` 或 enforce 流程。`low_competition_mode=observe` 只写入 `strategy_bucket` 和 `low_competition_metrics`，不会实盘挂单，也不会为无 active exposure 的低竞争 shadow 计划触发 AI/info-risk provider 请求；`enforce` 下同一 condition 的低竞争候选会替换 standard 候选，避免被全局去重吞掉，并要求默认 10U probe 的 competition share/competition multiple、账户与单 condition 挂单资金占比、预估 reward/100/day（默认至少 0.50）、退出深度、入场退出滑点、坏成交恢复天数、盘口历史样本（默认至少 30）、midpoint 波动和 top-of-book 跳变全部达标，同时要求 `ai_advisory_enabled=true` 且 `info_risk_enabled=true/info_risk_mode=enforce`，之后仍由既有 AI/info-risk cache gate fail closed；低竞争可要求高置信度 AI `allow` 并使用更严格的 info-risk avoid level。Worker 在 AI/info-risk gate 后写入低竞争 observation，snapshot 汇总最近 24 小时 shadow report（样本数、通过率、competition share 中位数、账户/单市场占比 P90、reward 中位/P90、退出深度倍数、中点波动 P95、退出滑点 P95、坏成交恢复天数 P95、AI/信息风险拦截、主策略重叠、高竞争混入占比和小额 enforce 建议）。独立 sleeve 市场数、开放订单和库存 cap 会在 live placement 使用；低竞争开放订单全局占比上限由 `low_competition_global_open_order_share_bps` 配置，默认 30%。低竞争待提交 BUY 在 POST 前会对同计划 `orderbook_token_ids` 和报价腿 token 做 1 秒 max-age last-look，缺任意必要盘口即 defer。低竞争开放 BUY 在常规撤单、待提交 intent、last-look 和事件驱动 hard-risk 撤单路径只共享通用硬风控（计划仍存在且 eligible、盘口缺失/为空、stale grace 后过期、best ask touch、kill switch 等），随后进入 `low_competition_cancel.rs` 专用撤单 gate，不再执行普通市场的 requote drift、min depth、bid rank、depth drop、fill velocity、mass cancel 或 requote age；事件撤单在订单自身或 companion token 更新时都会为当前低竞争 plan 补齐同计划盘口后复核。低竞争撤单 gate 使用可配置阈值，默认 competition share 低于入场阈值 80%、competition multiple 高于入场阈值 1.5 倍时，必须有 30 秒前历史盘口确认才撤；账户与单 condition 挂单资金占比超限立即撤；退出深度使用 `max(20U, 当前 condition BUY 剩余 notional * 2)`，退出滑点超过 2c 才撤，滑点无法估算或历史样本不足不会单独触发低竞争撤单。低竞争仍受全局 `max_markets`、`max_open_orders`、kill switch、盘口和账户外部 BUY notional 风控约束；report 不会自动切换配置。
...
- Rewards live tick 会在 AI/info-risk provider refresh 前对无开放订单/持仓的新 condition 执行 funding precheck，并在 provider 请求前应用 pre-LLM 硬 gate；当前可用资金放不下最低 rewards size 待补腿时先写入 funding reason 并跳过普通 provider 队列，低竞争 observe 且无 active exposure 的 shadow 计划只记录 observation 不调用模型，已有订单/持仓 condition 仍保留 provider 覆盖以继续风险管理。
...
- Rewards 低竞争市场 sleeve v2 已落地并默认关闭；系统会在启用 `observe/enforce` 后计算 `qualified_competition_usd`、默认 10U 探测资金的 `competition_share_bps`/`competition_multiple`、账户有效可用资金、低竞争开放 BUY 挂单占比、同 condition BUY 挂单占比、`estimated_reward_per_100_usd_day`、退出深度和盘口稳定性窗口，quote plan 会携带 `strategy_bucket` 与指标。低竞争候选不再用 Gamma liquidity 或 24h volume 作为准入门槛，旧低竞争 liquidity/volume 过滤字段只做兼容读取，保存/归一化后强制关闭并置 0；legacy `low_competition_max_competition_usd` 默认关闭。full tick 会持久化低竞争 observation，snapshot 会返回最近 24 小时 shadow report 和保守的小额 enforce 建议。只有 `enforce` 且竞争份额/资金占比/退出/稳定性指标、AI advisory、info-risk enforce 和既有 live 风控全部通过时才会按低竞争标签小额实盘下单；提交前 last-look 和挂单后常规/事件撤单路径都会要求低竞争计划必要盘口完整并重新应用低竞争 gate，companion token 更新会触发事件撤单复查；report 不会自动启用 enforce。
