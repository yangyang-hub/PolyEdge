...
- Rewards live tick 会在 AI/info-risk provider refresh 前对无开放订单/持仓的新 condition 执行 funding precheck；当前可用资金放不下最低 rewards size 待补腿时先写入 funding reason 并跳过普通 provider 队列，已有订单/持仓 condition 仍保留 provider 覆盖以继续风险管理。
