...
- `RewardBotConfig::production_live_drill_defaults()`：空库 Postgres 部署使用的保守生产演练 profile。保持 `enabled=false`，限制 2 个市场、6 个开放订单、单市场/单仓位/全局 `$10/$10/$25`，要求 1 cent effective edge，并启用深度骤降、成交速度和 mass-cancel 风控。通用 `Default` 继续作为纯计算和 in-memory 测试基线。
