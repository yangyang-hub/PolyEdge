...
| `rewards/` | 做市奖励：`RewardBotService`、`RewardBotStore`、质量过滤/排序、静态事件风险过滤（含地缘/军事事件关键词组：airspace closure / ceasefire / military strike / nuclear test / martial law / state of emergency 等）、结构化事件窗口 gate、首单 info-risk/quarantine gate、盘口指标/单边报价推荐、price-history candles、统一机会评分（竞争度、奖励密度、退出能力、稳定性和资金占用）与竞争倍数硬门槛（`opportunity_competition_hard_gate_enabled` + `opportunity_competition_hard_gate_multiple`，超过独立高阈值的市场直接不可挂，区别于仅 warning 的 `opportunity_max_competition_multiple`）、Rewards 做市商 EV/shadow/guarded 决策、标准/成交后合并 strategy profile、AI advisory 输入/决策/执行约束与 strategy hint（方向/挡位/金额 cap）、provider pre-LLM 硬过滤、Rewards 主/备 provider 并发配置、异步信息风险缓存、provider LLM 调用记录与每日统计、priority condition 列表、live-only 状态与订单分页查询、历史清理、events/fills/open_order_count 内存缓存、in-process command wake channel；`RewardBotStore` trait 拆在 `service/store.rs`，service 单元测试拆在 `service/tests.rs`，配置默认值/归一化/patch 逻辑拆在 `config_impl.rs`，运行时模型拆在 `runtime_models.rs`，事件窗口状态机拆在 `event_window.rs`，quote/selection/AI/strategy mode 枚举拆在 `quote_selection_models.rs`，AI 模型拆在 `ai_advisory_models.rs`，AI payload/定价上下文/1h candle 聚合 helper 拆在 `ai_advisory_payload.rs`，信息风险模型拆在 `info_risk_models.rs`，combined provider request/decision 拆在 `provider_models.rs`，provider 请求前硬过滤拆在 `provider_prefilter.rs`，deterministic 盘口选择 helper 拆在 `planner_selection.rs`，live 盘口 materializer 拆在 `planner_live.rs`，统一机会指标拆在 `opportunity_metrics.rs`，做市商 fair value/EV 决策拆在 `market_maker.rs`，snapshot 聚合拆在 `service_snapshot.rs`；旧低竞争字段、bucket、observation store 仅保留历史/API/DB 兼容 |
...
| `high_probability/` | 动态高概率市场定价研究 / Rewards fair value provider：`HighProbabilityService`、`HighProbabilityStore`、配置、market outcome 标签、rewards candle 输入、历史样本、结构化研究特征、分桶统计、研究 report、baseline walk-forward backtest report、基础退出规则对比、只读 observation 和 `FairValueEstimate` 快照；当前支持从本地 outcome 标签 + rewards candles 构建 first-touch 样本、刷新 bucket stats，并生成只读研究质量/基础回测报告和 fair value 诊断，不交易 |
...
- Market Maker：`latest_market_maker_fair_values` 从 `reward_market_fair_values` 读取当前启用模型版本的未过期 fair value 快照；`record_market_maker_decisions` 记录每个 quote leg 的 shadow/guarded EV 决策审计
...
- Orders/Positions/Events：完整 CRUD；订单支持 `RewardOrderListQuery` 后端分页并在 snapshot 中返回 `orders_page`，其中 `orders_status=filled` 会同时匹配 `status=filled` 和 `filled_size > 0` 的部分成交订单；live worker 可按 external Polymarket order id 查 managed order、用 fill id 做成交幂等，并通过 `latest_fill_at(account_id)` 查询账户最近 confirmed fill 时间；`RewardMergeIntent` 记录成交后合并 profile 中可配对 YES/NO 库存的合并意图，包含链上 tx hash、提交/确认/失败时间、失败原因和 retry_count；`pending/unsupported/submitted/completed` 仍计入 active 防重，store 提供 executable intent 查询和 submitted/failed 状态更新；`prune_history(cutoff)` 只清理 cutoff 之前的终态订单（`cancelled`/`filled`/`error`）、risk events 和 legacy 低竞争 observations，保留 `planned`/`open`/`exit_pending`、fills、positions 和 account state
...
- `RewardBotConfig.quote_mode=double|auto` 与 `selection_mode=observe|enforce` 控制确定性盘口选择。默认 `double + observe` 保持既有双边报价；`auto + enforce + dominant_single_side_enabled=true` 时，planner 只用 YES/NO 概率区间生成 `double` / `single_yes` / `single_no` / `none` 初步计划，不在计划构建阶段因盘口档位、退出深度、top1/top3 深度占比、HHI、`per_market_usd` 或 `quote_size_usd` 淘汰市场。live placement 通过 `materialize_reward_quote_plan_for_live_orderbook()` 读取当前盘口后再验证目标档位、rewards spread、token 实时 `best_ask - best_bid` 不超过 `max_market_spread_cents`、盘口集中度、退出深度、安全边际和实际 size/notional；双边目标档位、rewards spread、live token spread、touch ask 或安全边际不满足时，也在此阶段按实际目标档位价格尝试通过同一校验的单腿回退。`observe` 只在 quote plan 记录推荐模式和 `book_metrics`，不改变实际挂单。
- `RewardBotConfig.strategy_mode=rewards_only|market_maker_shadow|market_maker_guarded` 与 `market_maker_enabled` 控制 Rewards 做市商 EV 决策层。默认 `rewards_only + market_maker_enabled=false`，不改变既有 live 报价；shadow 模式读取 High Probability fair value 快照和现有机会评分，记录每条 BUY leg 的 pricing edge、reward EV、退出成本、逆向选择、不确定性与库存惩罚，但不改写 quote plan 或 live 下单；guarded 模式才会按正 EV、fair value confidence/uncertainty、pricing edge floor 和库存上限重定价/过滤 quote leg，并把结果写入 `quote_plan.market_maker` 和 `reward_market_maker_decisions`。guarded 默认关闭；当前未实现 fair-value-aware SELL manager、EV 变负撤单确认和 PnL attribution。
...
- AI advisory 定义低频模型判断的输入、输出和执行约束：`build_reward_ai_advisory_request()` 从开放订单、持仓和已通过初步 planner 筛选的 eligible 奖励市场、确定性计划、账户/仓位/开放订单、盘口 top levels、最近最多 24 小时的 5m price-history source candles 聚合出最多 24 根 1h candles 和 candle summary 构建完整 provider payload；payload 还显式携带 `pricing_context`（当前 best bid/ask、midpoint、binary midpoint sum、报价边际、是否触碰 ask、盘口确认年龄和 spread 合理性摘要）与 `provider_cache_policy`（TTL、正向 jitter 窗口、提前刷新窗口和最晚缓存复用时间），要求 provider 对整个 TTL horizon 输出 `allow_quote=true|false` 二值决策和 conservative `strategy_hint`（`quote_mode`、`bid_rank`、`max_condition_notional_usd`）。
  ...
  每个 condition 最多一次 provider 调用，请求只携带已启用且到期的 advisory/info-risk section；AI section 需要 `reward_market_books_available()` 校验所有报价 token 都已有非空 bids/asks，refresh 会先复用本轮 books 并通过 orderbook HTTP batch stale refresh 准备 AI 盘口，仅仍缺的 token 才按最多 10 个市场一批注册到 `rewards_ai_provider` 临时 orderbook source，下一批取消上一批，结束后清空；纯 info-risk 批次不等待临时盘口。provider 成功后只写 advisory/info-risk 缓存，不直接改写 `rewards_eligible`；后续 full tick 按缓存重新应用 gate，并在最终 quote plans 保存后统一刷新 `rewards_eligible` source。
  遇到明确限流、认证失败、常见 5xx 或 timeout/timed out 会停止本轮请求；普通瞬时传输错误继续下一个候选。默认 `ai_advisory_enabled=false`；配置 wire value 使用 `ai_provider=openai|anthropic` 和 `ai_request_format=openai_responses|openai_chat_completions|anthropic_messages`，后端兼容读取旧 `open_ai*`、`glm`、`deepseek` 拼写但序列化始终输出 `openai`；模型名包含 `glm` 或 `deepseek` 时会强制归一为 `openai_chat_completions`，避免配置到不支持的 Responses/Messages 路径。provider confidence 在 connector 解析时钳制到 `0..=1`；不允许挂单仍不能绕过市场质量、盘口和风控硬过滤；成交后退出策略由 `RewardBotConfig.post_fill_strategy` 决定，不再由 advisory `exit_policy` 覆盖。
...
- 报价计划构建阶段不再用 `quote_bid_rank` 缺档、目标价 rewards spread、盘口集中度、盘口价格预算、`per_market_usd` 或 `quote_size_usd` 过滤候选；计划腿可只是携带 YES/NO token 的占位元数据。planner 和 live materializer 读取盘口 midpoint/档位时按 `RewardOrderBook.confirmed_at` 判断新鲜度，`observed_at` 只表示盘口内容版本。live placement 准备创建订单时才用当前 orderbook materialize 真实腿：报价价格按 `quote_bid_rank` 选择目标盘口价，粗 tick 使用第 N 个不同买价，细 tick 使用从买一回退 `rank-1` 个 0.01 价格步长后的不高于目标价档位；双边计划优先要求 YES/NO 两腿都存在目标档位且通过 rewards spread、live token spread、touch ask 和安全边际校验，auto/enforce/dominant 下若双边的档位、spread、live token spread、touch ask 或安全边际不可行则尝试只挂通过同一校验的一条腿；单边计划只要求目标侧存在且 token 实时 `best_ask - best_bid` 不超过 `max_market_spread_cents`。缺少/过期新鲜盘口时不提交订单、保持计划 eligible 并等待 orderbook 订阅/缓存返回；没有可行单腿的目标档位价格距离各自中间价超过 `min(market rewards_max_spread, config.max_spread_cents)`，或 token 实时 spread 超过 `max_market_spread_cents` 等非 transient 验证失败时不下单且写入 `live_skip_until`/`live_skip_reason`，跳过标记默认 12 小时后失效以便奖励范围或盘口变化后重新评估；开放订单与最新目标档位价格相差超过 `requote_drift_cents` 时不会立即全量撤单，而是先经过 `requote_drift_confirm_sec` 历史盘口同向确认、`requote_drift_cooldown_sec` 最小挂单年龄和 `requote_drift_max_cancels_per_cycle` 单轮限速后才作为换价撤单候选。
...
- live tick 在候选盘口初读之后会先执行 funding precheck，再进入 AI/info-risk cache gate 和后台 provider refresh。订单同步和账户同步完成后，进入撤单、待提交 intent 和新挂单前，worker 会针对当前 open-like 订单与 eligible quote plan token 再做一次本地 cache / orderbook HTTP batch 刷新并合并到本轮 books，随后 materialize quote readiness、应用机会评分并保存 quote plan 快照；保存后才用最终 plans 注册 `rewards_eligible` orderbook source，避免 tick 内 I/O 耗时让初读盘口在 placement 阶段变旧，也避免控制台或周期注册任务读到未 live 验证的中间态。
...
- 当前 live 已具备 post-only token 买单提交、订单撤单、本系统托管订单 confirmed 成交同步、同轮多笔 trade 累计入账、成交后对侧 buy sibling 撤单以及 `ExitAtMarkup` / `HoldAndRequote` / `FlattenImmediately` sell 下单；既有 sell exit 不属于 sibling cancel 目标。`BalancedMerge` profile 默认关闭，开启后使用独立 `balanced_merge_*` 市场/订单/edge/库存配置生成 YES/NO 双边买单，成交后不生成 SELL 退出、不撤对侧 BUY；full tick 和 fast reconcile 会自动发现同 condition YES/NO 可配对库存，扣除 active merge intent 后写入 durable `RewardMergeIntent` 防重。`balanced_merge_auto_execute_enabled` 默认关闭；开启后新 intent 为 `pending`，worker 会在账户/持仓同步后读取 `pending/unsupported` intent，通过 `PolymarketChainConnector` 用配置私钥作为 Safe owner 签名并经 proxy wallet 提交 CTF `mergePositions`，提交成功后记录 tx hash 并标记 `submitted`，提交前失败标记 `failed`。执行前会跳过当前本地有同 token open-like SELL 的合并，并复核 YES/NO 当前库存均不小于 merge size。报价与退出订单先持久化 intent，买入 fill 与退出/合并 intent 同事务写入，提交结果未知时保持 open-like 锁定状态等待开放订单匹配恢复或人工对账，不自动重复提交；外部订单 404 会先保持 open-like 对账锁，若超过 5 分钟仍无 CLOB/Data API 成交证据则自动本地标记为 `cancelled`；提交结果未知订单在恢复查询确认 CLOB 无对应 open 订单后经过 `LIVE_SUBMISSION_UNKNOWN_CLOSE_AFTER_SECS`（默认 600 秒）宽限也会自动本地关闭（与 404 锁一致），但若完整 positions 快照显示提交后出现了对应 BUY 库存，则保留对账锁等待确认，避免把可能已成交订单误关。worker 还会用 CLOB open orders snapshot 反查普通 managed BUY：未归属但 token 可唯一映射到 active reward market 的开放 BUY 会被收养为 managed order，已有同 external id 的非 open 本地 BUY 会在 CLOB 仍 open 时重开；已提交、open-like 且无提交未知、404、pending cancel、post-only violation 等对账锁的 BUY 若已不在外部开放订单列表中，且同轮订单状态/成交对账可靠，才会本地关闭为 `cancelled`，释放开放挂单计数；如果 CLOB 撤单 rejected 明确表示订单找不到、已取消或已成交，也会以 Polymarket 为准关闭本地剩余量。`ExitAtMarkup` 价格以被吃买单原价加 `exit_markup_cents` 为基准并向上取整到 0.01 tick，默认加价为 0；`HoldAndRequote` 按被吃买单原价持久化 post-only SELL 退出 floor intent，之后继续正常报价；外部 positions 快照检测到尚无 open-like SELL、且 CLOB open-order snapshot 无同 token active SELL 的非零库存时，才会按该持仓 `avg_price` 创建 post-only 原价 SELL 退出 floor intent，即使该 condition 不在当前 rewards catalog。明确退出拒单使用有界退避并在达到最大拒绝次数后停止自动重试；若 CLOB 明确返回 token balance 已被 active orders 占用，worker 会关闭本地 exit intent 并以远端 active SELL 覆盖，避免重复提交；提交前低于 1 美元最小名义金额的退出单会进入短 reason 的 dust deferred 状态，每 300 秒重新评估但不重复拼接历史原因；`FlattenImmediately` 会持久化非 post-only flatten intent，提交前读取当前 best bid，best bid 不低于 floor 时按 best bid 用 FAK/taker SELL 尝试非亏损平仓，best bid 缺失或低于 floor 时按 30 秒退避保留本地 deferred exit 并重试。同 token 退出未完成时暂停新增买单。worker 会把外部 balance、账户开放买单总 notional 观测和完整 positions 快照写入 store，资金钱包地址优先使用 `FUNDER`，且 CLOB balance 为 0/失败时会用链上 pUSD 余额回填账户 snapshot；账户开放买单总 notional 与 managed BUY open-list 反查不受 confirmed fill 保护期影响，并用于估算未归属到本系统的外部 BUY notional 以保守限制新增买单；balance/positions 替换仍会根据 `latest_fill_at` 在 confirmed fill 后保护本地账户状态 120 秒。保护期结束后，成功 positions 快照原子替换该账户全部持仓，失败时保留上一版。外部 active SELL 仍不接管为 managed order；非 rewards 市场和无法唯一映射 token 的外部开放订单明细、submitted merge receipt 轮询确认以及奖励结算对账仍是缺口。
...
- Fair Values：`record_fair_value`、`list_fair_values`

**服务：** `HighProbabilityService` — 读取/保存配置、snapshot、记录已构建样本，通过 `build_reward_candle_samples()` 从本地 outcome 标签和 rewards price-history candles 构建 first-touch 样本，并通过 `refresh_bucket_stats()` 从已结算样本计算分桶统计。当前 `bucket_model.rs` 使用市场类型、价格 bucket、剩余时间 bucket、流动性 bucket 和 spread bucket 聚合样本，用 beta(1,1) posterior mean 作为保守 `fair_probability`，并输出胜率、期望 PnL、最大回撤、跌破阈值比例、平均持仓时间和推荐最高入场价。`features.rs` 生成价格路径、流动性、时间和风险标签研究特征，供样本、回测和后续模型解释使用。`backtest_report()` 使用较早 70% 已结算样本训练 bucket，并用较晚 30% 样本按当前 edge/缓冲规则模拟是否入场，输出基础交易数、胜率、PnL、ROI、最大回撤和 `settlement` / `take_profit_90` / `take_profit_95` / `stop_loss_70` / `stop_loss_60` 退出规则对比；`run_and_record_backtest()` 会把同一 baseline run、退出规则摘要和入场交易明细持久化到 backtest 表。`observe_candidates()` 从 store 候选和 worker 提供的 orderbook quote 计算 `allow/reject/skip` observation：匹配当前模型版本 bucket，检查 net edge、最低置信度、推荐最高入场价、spread、ask depth 和排除风险标签后写入 `high_probability_observations`。`refresh_fair_values()` 在 `fair_value_enabled=true` 时读取候选、当前 orderbook quote 和 bucket stats，按 5 维 bucket 回退链（精确到全局先验）融合市场隐含概率与历史 base rate，写入 `reward_market_fair_values` 的 `fair_yes_low/mid/high`、confidence、uncertainty、reason_codes、input_hash 和 live_eligible。该回测、observe 和 fair value provider 都是研究/只读路径，不模拟完整订单簿成交、滑点队列或多阶段执行成本，也不下单。

**当前语义：**
- 已落地基础模型、service/store port、rewards candle first-touch 样本构建、结构化研究特征、保守分桶统计、只读 research report、baseline walk-forward backtest report、基础退出规则对比、一次性 observe 扫描与 observation 写入、持久化 baseline backtest runs/trades、fair value 快照生成/持久化、只读 API snapshot/config/buckets/report/backtests/backtest-runs/fair-values 和单元测试。
- 不抓外部数据；样本构建只消费数据库内 `reward_market_candles`、`high_probability_market_outcomes` 和 `markets`。
- API 可读取 `/api/v1/high-probability` snapshot、`/api/v1/high-probability/config`、`/api/v1/high-probability/buckets`、`/api/v1/high-probability/report`、`/api/v1/high-probability/backtests`、`/api/v1/high-probability/backtest-runs`、`/api/v1/high-probability/backtest-runs/{run_id}/trades` 和 `/api/v1/high-probability/fair-values`；report 从现有样本与当前模型版本 bucket stats 计算样本覆盖、胜负分布、合格分桶数、加权胜率/期望和数据提示，不新增外部 I/O；backtests 当前返回即时 70/30 walk-forward baseline 报告，backtest-runs 读取已持久化 run/trade，fair-values 只读取未过期当前 model_version 快照。Worker CLI `import-high-probability-outcomes <path>` 可通过 service upsert 本地 outcome 标签；`build-high-probability-samples-once [limit]` 从 rewards candles 构建样本；`refresh-high-probability-buckets-once` 读取已有 `high_probability_samples` 并刷新 `high_probability_bucket_stats`；`run-high-probability-backtest-once` 运行并持久化 baseline 回测；`observe-high-probability-once [limit]` 读取本地候选和 orderbook 服务缓存后写入 observations；`refresh-high-probability-fair-values-once [limit]` 在配置开启时写入 fair value 快照；API 内嵌 worker runtime 可通过 `POLYEDGE_WORKER__POLL_HIGH_PROBABILITY_OBSERVE=true` 或 `POLYEDGE_WORKER__POLL_HIGH_PROBABILITY_FAIR_VALUES=true` 定时运行同类只读流程，默认关闭。
- 全市场 price-history/outcome producer、完整执行成本/多阶段退出回测、Phase 4 校准与漂移监控仍未实现；Rewards 已能在 shadow/guarded 做市商决策中引用 `reward_market_fair_values`，但该引用默认关闭且仍不代表 High Probability 拥有交易执行权；该模块不再作为独立 paper/live 执行器推进。
...
- Rewards live 模式已接入质量硬过滤与综合排序、post-only token 买单、撤单、本系统托管订单成交同步、成交后现金/库存/PnL 更新、可持续重试的 exit/flatten sell、CLOB open-order 反查、可映射 active rewards BUY 的收养/重开、外部 active SELL token 覆盖库存补退出、外部余额/完整持仓快照、managed order scoring 和 UTC 当日账户级 maker rewards 同步（聚合端点优先、明细端点 fallback）；新增买单会把未归属到本系统 managed order 的外部 BUY notional 从可用资金中保守扣除，并要求 orderbook 盘口距离 stale 边界仍有余量，近期 BUY 的 stale-only 撤单有短暂 grace 以吸收 registry/poll 抖动。外部 SELL 仍不接管为 managed order；非 rewards 市场和无法唯一映射 token 的外部开放订单明细与奖励结算对账仍待完成。
...
- High Probability Pricing 已新增后端 research foundation 并收敛为 Rewards fair value/base-rate provider：application service/store、Postgres/in-memory store、迁移、只读 API snapshot/config/buckets/report/backtests/backtest-runs/fair-values、`import-high-probability-outcomes`、`build-high-probability-samples-once`、`refresh-high-probability-buckets-once`、`run-high-probability-backtest-once`、`observe-high-probability-once`、`refresh-high-probability-fair-values-once` CLI 和默认关闭的自动 observe/fair value worker；当前可导入本地 outcome JSON 标签，并从本地 outcome 标签 + rewards candles 构建 first-touch 样本、刷新分桶统计、生成只读研究报告、即时 baseline walk-forward 回测报告和基础退出规则对比，持久化 baseline backtest runs/trades，并基于本地 rewards candle 候选 + orderbook 服务缓存写入只读 observations 和 `reward_market_fair_values` 快照。Rewards 做市商初版已可按配置读取这些 fair values 生成 shadow/guarded EV 决策并写入审计，但默认仍关闭 guarded live。当前没有 LLM/provider 调用路径；全市场 price-history/outcome producer、完整执行成本/多阶段退出回测、Phase 4 校准与漂移监控仍未实现。
