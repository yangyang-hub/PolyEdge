...
- Rewards SELL 退出单提交前会按当前 best bid 与非亏损退出 floor 判断：best bid 不低于 floor 时走非 post-only FAK/taker SELL，best bid 低于 floor 时递延，避免 crossing post-only 拒单和亏损卖出；信息风险 enforce 不再仅凭 `risk_type=imminent_resolution` 拦截，只有 `resolution_imminent=true`、官方结果或命中的风险等级才会阻断。
