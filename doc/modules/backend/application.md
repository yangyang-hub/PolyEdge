...
- Smart Money Intelligence 已新增后端 foundation：数据库 schema、application service/store、Postgres/in-memory store、snapshot/config API、`scan-smart-money-once` CLI 和可选 `poll-smart-money`/runtime 定时扫描；当前能从 Data API leaderboard 和 active copytrade tracked wallets 生产候选，扫描候选钱包生成画像、评分和源交易，并基于 orderbook 服务缓存生成确定性 observe/rejected 信号和 deterministic decision 审计；开启 signal advisory 时 worker 可为 observe 信号构造 advisory payload/input_hash、检查缓存，并在 provider key 存在时调用 Smart Money signal advisory connector 保存三态 advisory。完整全网 discovery、独立 Smart Money provider 配置、纸面模拟和实盘执行尚未实现。
- High Probability Pricing 已新增后端 research foundation：application service/store、Postgres/in-memory store、迁移、只读 API snapshot/config/buckets/report/backtests/backtest-runs、`import-high-probability-outcomes`、`build-high-probability-samples-once`、`refresh-high-probability-buckets-once`、`run-high-probability-backtest-once`、`observe-high-probability-once` CLI 和默认关闭的自动 observe worker；当前可导入本地 outcome JSON 标签，并从本地 outcome 标签 + rewards candles 构建 first-touch 样本、刷新分桶统计、生成只读研究报告、即时 baseline walk-forward 回测报告和基础退出规则对比，持久化 baseline backtest runs/trades，并基于本地 rewards candle 候选 + orderbook 服务缓存写入只读 observations。全市场 price-history/outcome producer、完整执行成本/多阶段退出回测、paper/live guarded 执行尚未实现。
