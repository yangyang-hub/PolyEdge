...
- 低竞争 rewards sleeve 已实现 v2，默认 `low_competition_mode=off`。它使用独立 `standard` / `low_competition` candidate profile，不全局降低主策略 `min_market_liquidity_usd` / `min_market_volume_24h_usd`；后端配置归一化会强制关闭旧 `low_competition_candidate_*_filter_enabled` 开关并把低竞争 liquidity/24h volume 阈值置 0，因此低竞争 profile 不以 Gamma liquidity 或 24h volume 作为准入门槛，而是共享 open/tradable、高歧义、唯一 YES/NO、结算时间、Gamma spread、数据新鲜度和 FDV/launch/token/official-result 事件风险硬过滤，并在候选查询中按低竞争奖励密度/低流动性优先排序。为识别流动性低但盘口竞争极激烈的“伪低竞争”候选（候选 SQL 阶段 markets 表无盘口深度，无法预筛），`build_low_competition_metrics` 会在拿到真实盘口后按 `low_competition_candidate_max_competition_multiple`（默认 20，归一化时不低于正式 gate 阈值）给 competition_multiple 超标的候选打 `not_low_competition` 分类标签，只用于下游 prewarm/observation 降级和 shadow report 的“高竞争混入占比”统计，不进入正式 gate 的 rejection_reasons，不影响 `eligible_for_low_competition` 或 enforce 流程。`low_competition_mode=observe` 只写入 `strategy_bucket` 和 `low_competition_metrics`，不会实盘挂单，也不会为无 active exposure 的低竞争 shadow 计划触发 AI/info-risk provider 请求；`enforce` 下同一 condition 的低竞争候选会替换 standard 候选，避免被全局去重吞掉，并要求默认 10U probe 的 competition share/competition multiple、账户与单 condition 挂单资金占比、预估 reward/100/day、退出深度、盘口历史样本和 midpoint 波动全部达标，同时要求 `ai_advisory_enabled=true` 且 `info_risk_enabled=true/info_risk_mode=enforce`，之后仍由既有 AI/info-risk cache gate fail closed。Worker 在 AI/info-risk gate 后写入低竞争 observation，snapshot 汇总最近 24 小时 shadow report（样本数、通过率、competition share 中位数、账户/单市场占比 P90、reward 中位/P90、退出深度倍数、中点波动 P95、退出滑点 P95、AI/信息风险拦截、主策略重叠、高竞争混入占比和小额 enforce 建议）。独立 sleeve 市场数、开放订单和库存 cap 会在 live placement 使用；低竞争待提交 BUY 在 POST 前会对同计划 `orderbook_token_ids` 和报价腿 token 做 1 秒 max-age last-look，缺任意必要盘口即 defer。低竞争开放 BUY 在常规撤单、待提交 intent、last-look 和事件驱动 hard-risk 撤单路径只共享通用硬风控（计划仍存在且 eligible、盘口缺失/为空、stale grace 后过期、best ask touch、kill switch 等），随后进入 `low_competition_cancel.rs` 专用撤单 gate，不再执行普通市场的 requote drift、min depth、bid rank、depth drop、fill velocity、mass cancel 或 requote age；事件撤单在订单自身或 companion token 更新时都会为当前低竞争 plan 补齐同计划盘口后复核。低竞争撤单 gate 使用比入场更宽的竞争阈值：competition share 低于入场阈值 80%、competition multiple 高于入场阈值 1.5 倍时，必须有 30 秒前历史盘口确认才撤；账户与单 condition 挂单资金占比超限立即撤；退出深度使用 `max(20U, 当前 condition BUY 剩余 notional * 1.5)`，退出滑点超过 3c 才撤，滑点无法估算或历史样本不足不会单独触发低竞争撤单。低竞争仍受全局 `max_markets`、`max_open_orders`、kill switch、盘口和账户外部 BUY notional 风控约束；report 不会自动切换配置。
