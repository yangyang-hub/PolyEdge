...
| `src/orderbook.rs` | 目标 token 集查询、`PolymarketOrderBookConnector` REST poll、内存盘口 cache |
...
常规业务写请求要求 `Idempotency-Key`，以 actor user + scope + request hash 持久化首次完整响应；同时只接受 `X-PolyEdge-CSRF-Token` 并校验 `Origin == POLYEDGE_PUBLIC_ORIGIN`。不接受 Authorization/Bearer 身份或旧 CSRF header 别名。登录、激活、登出和重新认证不进入业务幂等层；管理员用户更新使用普通幂等重放，创建用户与重签激活 token 使用不回放明文 token 的冲突语义。危险写入类别为：

- `wallet_trading_enable`
- `wallet_secret_rotate`
- `execution_submit`
- `order_cancel_force`
- `user_admin`
- `cash_flow_record`
