...
| `packages/api/src/lib.rs` | 路由组装入口：`build_app(state: AppState) -> Router`（~353 行） |
...
| `packages/api/src/handlers/execution_lists.rs` | 列表：orders、drafts、trades、execution requests |
...
| `packages/api/src/handlers/mappers.rs` | DTO 映射辅助；保留 connector callback 需要的执行/持仓/风险映射 |
...
- 当前 REST 端点覆盖市场、事件/证据、新闻、订单/成交、pricing、rewards、funding、copytrade、wallet analysis、runtime config、system mode、connector callback 和 orderbook 代理
