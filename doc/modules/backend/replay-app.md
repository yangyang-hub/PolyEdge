...
- `RewardDecisionReplayFixture` / `RewardDecisionReplayResult`：由 application crate 定义。V1 保留完整 final state/expected plans；V2 使用决策窗口 top-of-book history、final delta 和 expected normalized plan SHA-256。

## 当前状态

- `cargo run -p polyedge-replay -- --run-id <RUN_ID>` 输出指定 run 的 JSON 审计报告；省略参数时读取最新 run。
- `cargo run -p polyedge-replay -- --fixture <FIXTURE.json>` 按 `schema_version` 自动读取 V1/V2；缺省版本字段按 V1 兼容。V1 比较完整 expected plans，V2 比较 normalized plan hash 并输出 reason-code diff。
...
- Full tick 默认异步保存 V2 紧凑 fixture；队列满、安全检查/体积拒绝或保存失败都不会阻断 live tick。历史 V1 reader 仍保留。盘口 fill risk、exit cost 与 cancel churn 仿真仍未实现。
