...
| `llm_provider.rs` | LLM provider 全局单并发闸门：Rewards AI advisory、Rewards 信息风险和 Smart Money signal advisory 的实际外部模型 HTTP 调用共享同一个进程内 semaphore |
| `openai_compat.rs` | OpenAI-compatible provider helper：root base URL 自动补 `/v1`、已带 `/vN` 的 provider base 保持原样，请求同时携带 Bearer 与 `api-key` 认证头，模型名包含 GLM/DeepSeek 时的 Chat Completions JSON mode 差异、glm-4.7 思考模型注入 `thinking:{type:disabled}`、provider 文本响应候选 JSON 提取与错误 preview |
| `reward_provider.rs` | Rewards combined provider 连接器：单市场一次 LLM 调用可同时返回 AI advisory 与 info-risk section，支持 OpenAI Responses、OpenAI-compatible Chat Completions（含 GLM/DeepSeek 模型名特例）、Anthropic Messages 和可选 OpenAI Responses web search；glm-4.7 思考模型在 Chat Completions 路径关闭 thinking 并把 completion token 预算设为 8192，避免 reasoning_content 烧光预算导致 `content` 空被 `max_tokens` 截断（`finish_reason: length`） |
...
- Completion 预算为 2048 tokens，provider 请求温度固定为 0。该 connector 不直接访问 Polymarket；它只接收 application 层基于数据库、orderbook、quote plan、账户状态和策略配置构建的 payload。
