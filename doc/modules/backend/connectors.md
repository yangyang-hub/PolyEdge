...
`polyedge_connectors` crate 实现所有外部系统的适配器：Polymarket CLOB（交易）、Gamma API（市场数据）、Data API（钱包活动）、Order Book（盘口）、Rewards API（奖励市场）、Rewards combined provider（AI advisory + info-risk）、Smart Money signal advisory、RSS 新闻源，以及内置的 Paper Trading 执行器。
...
| `openai_compat.rs` | OpenAI-compatible provider helper：root base URL 自动补 `/v1`、已带 `/vN` 的 provider base 保持原样，请求同时携带 Bearer 与 `api-key` 认证头，模型名包含 GLM/DeepSeek 时的 Chat Completions JSON mode 差异、Agnes Chat Completions 端点归一化、glm-4.7 思考模型注入 `thinking:{type:disabled}`、provider 文本响应候选 JSON 提取与错误 preview |
| `reward_provider.rs` | Rewards combined provider 连接器：单市场一次 LLM 调用可同时返回 AI advisory 与 info-risk section，支持 OpenAI Responses、OpenAI-compatible Chat Completions（含 GLM/DeepSeek/Agnes 模型名特例）、Anthropic Messages 和可选 OpenAI Responses web search；glm-4.7 思考模型在 Chat Completions 路径关闭 thinking 并把 completion token 预算设为 8192，避免 reasoning_content 烧光预算导致 `content` 空被 `max_tokens` 截断（`finish_reason: length`） |
| `reward_ai.rs` | Rewards AI advisory schema / parser helper：二值 `allow_quote`、`strategy_hint`、旧 `suitability` fail-closed 兼容 |
| `reward_ai_tests.rs` | Rewards AI advisory parser tests：二值 `allow_quote`、`strategy_hint` 和旧响应兼容 |
| `reward_info_risk.rs` | Rewards 信息风险 schema / parser helper：二值 `allow_quote` 和旧 taxonomy response 兼容 |
| `reward_info_risk_tests.rs` | Rewards 信息风险 parser tests：二值 `allow_quote`、旧响应兼容和 confidence clamp |
| `smart_signal_advisory.rs` | Smart Money signal advisory 连接器：OpenAI Responses、OpenAI-compatible Chat Completions（含 GLM/DeepSeek/Agnes 模型名特例）、Anthropic Messages，解析 `allow|observe|reject` 三态建议 |
...
- **`PolymarketGammaMarket`**：id、slug、question、category、status、best_bid/ask/mid_price、`liquidity_usd`、volume_24h、`start_at`、`end_at`、`event_start_at`、`event_end_at`、`has_reviewed_dates`、ambiguity_level、tradability_status、condition_id、yes/no_asset_id、原始 outcome token ids/outcomes/prices 等
...
- 流动性优先解析 Gamma `liquidityClob`，缺失时回退 `liquidity`；结算时间解析 `endDateIso`/`endDate`，日期候选解析 `startDateIso`/`startDate` 和 `events[].startDate/endDate`，并保留 `hasReviewedDates` 供 orderbook market sync 写入 rewards event-window 候选。Gamma 日期默认只作为低/中置信候选，不由 connector 提升为 hard gate。歧义等级只在 market/event 提供显式 `resolutionSource` 时为 Low，仅 description 可用时为 Medium，两者都缺失时为 High，避免把描述文本误当成明确结算来源。
...
- **`RewardProviderConnector`**：`base_url` + API key + reqwest client，供 rewards worker 对单个市场一次性评估 AI advisory 和/或 info-risk。请求是 `RewardProviderRequest`，其中 `advisory` 与 `info_risk` section 可分别为 `None`；connector 只发送本次需要且已到期的 section，响应解析为 `RewardProviderDecision` 后由 worker 分别写入 `reward_market_advisories` / `reward_market_info_risks`。
- 支持三种请求格式：`openai_responses` 调用 OpenAI-compatible `{base}/responses` 并使用 JSON schema structured output；`openai_chat_completions` 调用 `{base}/chat/completions`；`anthropic_messages` 调用 `{base}/v1/messages`。OpenAI-compatible 路径会在 root base URL 自动补 `/v1`，但已带 `/vN` 的 provider base（如 GLM `/api/coding/paas/v4`）保持原样，并同时发送 `Authorization: Bearer` 与 `api-key`。模型名包含 `glm` 或 `deepseek` 时强制 Chat Completions，并使用更通用的 `response_format=json_object` + `max_tokens`；模型名包含 `agnes` 时也强制 Chat Completions，但保留 strict `json_schema` 与 `max_completion_tokens`。
- Combined schema 是一个 JSON object，按请求 section 要求返回 `advisory` 和/或 `info_risk`。`advisory` schema 仍是 `allow_quote`、`confidence`、`strategy_hint`、`reasons`、`metrics`；`info_risk` schema 仍是 `allow_quote`、`confidence`、`summary`、`sources`、`metrics`。解析层复用 `reward_ai.rs` 与 `reward_info_risk.rs` helper，因此仍兼容旧 `suitability` / 旧 risk taxonomy 响应，confidence 会钳制到 `0..=1`。
- OpenAI Responses 仅在请求包含 info-risk 且 `POLYEDGE_REWARDS__INFO_RISK_WEB_SEARCH_ENABLED=true` 时附加 `web_search_preview` tool；advisory-only、Chat Completions 和 Anthropic Messages 路径不会发送 web search。
- Completion 预算为 8192 tokens，provider 请求温度固定为 0。该 connector 不直接访问 Polymarket；它只接收 application 层基于数据库、orderbook、quote plan、账户状态和策略配置构建的 payload。
...
- 支持三种请求格式：`openai_responses`、`openai_chat_completions`、`anthropic_messages`。OpenAI-compatible 路径复用 `openai_compat.rs` 的 base URL 归一化、Bearer + `api-key` 双认证头、GLM/DeepSeek Chat Completions JSON mode、Agnes Chat Completions 端点归一化和 token 字段差异；provider 请求温度固定为 0。
...
- Rewards combined provider connector 和 Smart Money signal advisory connector 已支持 OpenAI Responses、OpenAI-compatible Chat Completions（含 GLM/DeepSeek/Agnes 模型名特例）和 Anthropic Messages 三类路径；connectors 不再维护全局 LLM 单飞闸门，实际外部 provider 并发由 worker 功能池控制：Rewards 按 `RewardBotConfig` 的主/备 provider 并发限制执行，Smart Money signal advisory 按 `SmartMoneyConfig` 的独立并发限制执行。OpenAI-compatible base URL 可配置为根地址、`/v1` 地址或 GLM 这类 `/v4` versioned base，connector 会对 root 自动补 `/v1`、保留 `/vN`，并兼容 Bearer / `api-key` 认证头；MiMo provider 已验证 root gateway + `openai_chat_completions` + JSON schema 可用，`openai_responses` 会返回 provider 未实现错误；模型名包含 `glm` 或 `deepseek` 时强制归一到 `openai_chat_completions`，Chat 请求使用 `response_format=json_object` 与 `max_tokens`，其它 OpenAI-compatible Chat 请求仍使用 JSON schema 与 `max_completion_tokens`；模型名包含 `agnes` 时也强制归一到 `openai_chat_completions`，但仍使用 strict JSON schema 和 `max_completion_tokens`。GLM 可用 `POLYEDGE_REWARDS__AI_OPENAI_BASE_URL=https://open.bigmodel.cn/api/coding/paas/v4` + `POLYEDGE_REWARDS__AI_MODEL=glm-4.7`，DeepSeek 可用 `POLYEDGE_REWARDS__AI_OPENAI_BASE_URL=https://api.deepseek.com` + `POLYEDGE_REWARDS__AI_MODEL=deepseek-v4-flash`，Agnes 可用 `POLYEDGE_REWARDS__AI_OPENAI_BASE_URL=https://apihub.agnes-ai.com/v1` + `POLYEDGE_REWARDS__AI_MODEL=agnes-2.0-flash`；GLM/DeepSeek 已用测试 key 对当前请求形态做过鉴权 smoke test（均 HTTP 200，返回可解析 JSON object 字符串），Agnes 已用测试 key 对 Chat Completions strict JSON schema、Rewards combined schema 和 Smart Money signal advisory schema 做过 smoke test（均 HTTP 200，返回可解析 JSON）。正式运行密钥仍只通过 worker 环境变量注入。Rewards combined provider 给 8192 completion token 预算，且 glm-4.7 系列思考模型（含 `glm-4.7-flashx`）会在 Chat Completions 请求注入 `thinking:{type:disabled}`，避免思考链烧光预算导致 `content` 为空被 `max_tokens` 截断（`finish_reason: length`）；Smart Money signal advisory 仍给 2048 completion token 预算且未注入 thinking；模型密钥来自 worker 环境变量，provider 请求温度固定为 0。Rewards combined provider schema 按 section 输出 advisory 的 `allow_quote` + conservative `strategy_hint` 以及 info-risk 的 `allow_quote`；Smart Money signal advisory provider schema 输出 `allow|observe|reject` 三态建议。两者都会映射到各自内部模型字段；解析层会从 provider 文本中提取并校验候选 JSON 对象，provider confidence 输出会在解析时钳制到 `0..=1`。Rewards combined provider 失败不终止 live tick；AI 开启或 info-risk enforce 时缺缓存仍按既有 fail-closed/cache gate 处理；Smart Money signal advisory provider 失败只记录 warning/失败调用，不影响 deterministic 信号生成或已有缓存。信息风险 connector 的 OpenAI web search 工具默认关闭，仅在显式环境变量开启时使用。
