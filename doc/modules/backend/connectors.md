...
`polyedge_connectors` 只封装 V3 仍需要的 Polymarket 协议：指定 token 盘口、每钱包认证 CLOB 订单/余额，以及 Data API 钱包持仓读取。它不负责选市场、不读取数据库、不执行业务策略。

V3 活动路径不包含 Gamma、RSS/Atom、AI/provider、info-risk、Polygon chain、独立 orderbook service client、paper executor、rewards catalog 扫描或 price-history 数据源。
...
- connector 不记录 private key、API secret/passphrase、认证 header 或完整敏感响应。
...
- market-channel WebSocket 未接入 V3 targeted orderbook，当前使用 REST poll。
