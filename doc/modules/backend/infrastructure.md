...
| `RewardsSettings` | enabled、poll_interval、OpenAI-compatible/Anthropic API key 与 base URL、AI model/timeout/min confidence、可选备用 provider（第二个独立 provider/model/key/URL；GLM/DeepSeek/Agnes 通过模型名识别）、信息风险扫描间隔/每轮 condition cap/置信度/web search 开关等 |
