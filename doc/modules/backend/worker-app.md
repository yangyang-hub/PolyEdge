...
| `worker/rewards/provider_refresh_batch.rs` | rewards 主 provider refresh 的批量请求 helper：按配置批量请求 AI advisory / info-risk，AI 请求上限按缓存过滤后的实际 provider request 消耗，batch 失败或漏项后的单市场回退也受同一 cap 约束，逐项保存并记录实际 provider 调用 |
...
| `worker/rewards/provider_refresh.rs` | rewards AI advisory / 信息风险 provider refresh：full tick 后分别启动 AI advisory 与 info-risk 后台 task，各自单实例、各自 pre-LLM gate，并按 rewards poll interval 派生 wall-clock timeout 释放单实例锁；AI refresh 使用 cache gate 前 deterministic cycle 构建稳定 input hash，AI provider request cap 在缓存过滤后消耗；AI 盘口使用 `rewards_ai_provider` 临时 source 每批最多 10 个市场，info-risk 不再占用 AI 临时盘口批次；实际外部调用经 connectors 全局 LLM 单并发闸门并写入 `llm_calls` |
