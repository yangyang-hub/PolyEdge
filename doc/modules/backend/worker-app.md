...
- rewards full tick 已读取 Gamma `markets.category` 作为候选评分输入；命中 `preferred_categories` 时只增加候选排序分，不绕过市场质量、盘口和风控硬过滤。AI advisory 已接入 full tick：按 input hash 缓存、支持 OpenAI/Anthropic provider；AI 开启后只有高置信度 `allow` 决策会放行新增挂单，provider 未配置、失败、缺缓存或低置信度都会让原本 eligible 的计划变为不可挂，且不会放宽任何硬过滤；provider confidence 会被钳制到 `0..=1`。信息风险已作为独立异步 worker 接入：扫描结果落库，live tick 只读缓存并在 enforce 模式下过滤高风险/临近结算/官方结果盘口；新增买单会保守扣除未归属到本系统 managed order 的外部 BUY notional。
