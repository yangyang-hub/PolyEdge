...
| `worker/rewards/account_sync.rs` | rewards 外部余额、CLOB open-order 反查、完整持仓、订单 scoring 与 UTC 当日账户级 rewards 聚合同步 |
...
无待处理控制命令时：
    fetch_reward_bot_inputs() // 获取奖励市场 + 盘口
        → prepare_live_cycle()
        → 只应用已缓存 AI advisory，并在后台单实例刷新缺失 advisory 缓存
...
- rewards full tick 已读取 Gamma `markets.category` 作为候选评分输入；命中 `preferred_categories` 时只增加候选排序分，不绕过市场质量、盘口和风控硬过滤。AI advisory 已接入 full tick：live tick 只读已有 cache 并 fail closed，后台 provider refresh 按 input hash 异步填充 `reward_market_advisories`；AI 请求候选限定为 deterministic planner 原本 eligible 且仍缺少有效 advisory 的 condition，AI 开启后只有高置信度 `allow` 决策会放行新增挂单，provider 未配置、失败、缺缓存或低置信度都会让原本 eligible 的计划变为不可挂，且不会放宽任何硬过滤；provider confidence 会被钳制到 `0..=1`。信息风险已作为独立异步 worker 接入：扫描结果落库，live tick 只读缓存并在 enforce 模式下过滤高风险/临近结算/官方结果盘口；新增买单会保守扣除未归属到本系统 managed order 的外部 BUY notional。
