...
Worker 代码现在同时提供共享库和兼容 CLI。`polyedge-api` 在同一进程内启动 `WorkerRuntime`，运行数据库维护、新闻采集/提升、执行分发、订单对账、奖励机器人、copytrade 钱包跟踪/分析和 orderbook token 注册；旧信号重算和套利雷达 worker 已移除。`polyedge-worker` 二进制继续提供维护/调试子命令，但 Docker 不再部署独立常驻 worker 容器。
...
| `worker/rewards/live_pending.rs` | rewards live 持久化 intent 提交、BUY 提交前 last-look（低竞争覆盖全计划 token）、SELL 退出持仓裁剪/无仓位关闭、开放订单匹配恢复和未知结果锁定 |
...
| `worker/rewards/live_cancel.rs` | rewards live 撤单候选与撤单原因分流：通用硬风控、低竞争专用撤单分支、普通市场 depth/rank/history/requote 规则 |
| `worker/rewards/live_risk.rs` | rewards live placement 风控：资金/库存 cap、订单创建准入、depth/rank/history/requote helper |
...
| `worker/rewards/event_cancel.rs` | rewards orderbook 事件驱动 hard-risk 撤单快路径：独立 task 消费活跃 token 更新，只做 cancel-only 风控，不跑重型同步/重挂/换价；当前低竞争 plan 的订单会在自身或 companion token 更新时补齐同计划盘口并复核竞争指标 |
...
| `worker/rewards/provider_refresh_batch.rs` | rewards 主 provider refresh 的批量请求 helper：按配置批量请求 AI advisory / info-risk，逐项保存，记录实际 provider 调用，漏项或错配回退单市场 |
...
| `worker/rewards/provider_refresh.rs` | rewards AI advisory / 信息风险 provider refresh：full tick 后分别启动 AI advisory 与 info-risk 后台 task，各自单实例、各自候选 cap 和 pre-LLM gate；AI 盘口使用 `rewards_ai_provider` 临时 source 每批最多 10 个市场，info-risk 不再占用 AI 临时盘口批次；实际外部调用写入 `llm_calls` |
| `worker/rewards/provider_refresh_candidates.rs` | rewards provider refresh 候选 condition 归一化、开放订单/持仓优先级和普通/低竞争约 2:1 混排 helper |
...
无待处理控制命令时：
    fetch_reward_bot_inputs() // 获取奖励市场 + 盘口
        ...
        → 低竞争 sleeve 计算默认 10U 探测资金的竞争份额/竞争倍数、账户与单市场挂单资金占比、预估 reward share、退出深度和盘口稳定性指标
        → provider refresh 前执行 live funding precheck 和 pre-LLM 候选硬过滤：无开放订单/持仓的新 condition 若当前可用资金放不下最低 rewards size 待补腿，先标记 funding reason；低竞争 observe 且无敞口的 shadow 计划只记录 observation；低竞争 enforce 必须先通过指标
        → 只应用已缓存 AI advisory，并在后台分别单实例刷新缺失 AI advisory 与信息风险缓存（两条队列各自开放订单/持仓优先；无敞口候选先由 pre-LLM gate 标为 standard 或 low-competition，再按普通候选与通过 gate 的低竞争 pre-AI 市场约 2:1 混排，低竞争未通过 gate 不会回退为普通候选；AI advisory 使用 `rewards_ai_provider` 临时 orderbook source，每批最多 10 个市场，切换下一批前取消上一批；AI advisory / info-risk provider 请求分别受 rewards 配置 `ai_advisory_batch_size` / `info_risk_batch_size` 控制，1 为逐市场，>1 批量请求并对漏项/错配回退单市场；每次实际外部 provider HTTP 调用写入 `llm_calls`，批量请求计为 1 次调用；仅在该市场所有报价 token 盘口都已发布后才请求 AI 并写缓存，请求 payload 会读取最近最多 24 小时 5m price-history source candles 并聚合为最多 24 根 1h candles，同时带当前盘口定价合理性和 TTL/cache policy，cache key 只看已完成小时级摘要；缺盘口的市场本轮跳过请求、不写缓存，等盘口到达后再评估，避免缓存“无盘口不允许挂单”结果长期卡住市场）
        ...
        → orderbook stream active-token event：独立 cancel worker 只读更新 token 的活跃盘口，低竞争 companion token 更新会触发同 condition 订单复查，随后直接执行 hard-risk cancel-only 检查
...
低竞争 rewards sleeve 已实现 v2，默认关闭。worker 会从 `RewardBotService` 读取标准候选和低竞争候选 profile；低竞争 profile 后端归一化强制关闭旧 liquidity/24h volume 过滤开关并把阈值置 0，不以 Gamma liquidity 或 24h volume 作为准入门槛，仍共享市场安全硬过滤，候选查询按低竞争奖励密度和较低盘口资金优先排序；`enforce` 下同一 condition 的低竞争候选会替换 standard 候选，避免被去重吞掉。full tick 在读取候选后先用进程内 `LowCompetitionProbeState` 选择最多 10 个低竞争候选市场注册到 `rewards_low_competition_probe` source，批次在两腿盘口历史达到 `low_competition_min_book_samples`、候选失效或 5 分钟超时后轮转；随后 `prepare_live_cycle()` 用 orderbook 服务提供的当前盘口、账户状态、开放订单和 worker 本地盘口历史计算 `planned_notional_usd`、`qualified_competition_usd`、默认 10U probe 的 `competition_share_bps`/`competition_multiple`、`account_effective_available_usd`、低竞争与 condition after-plan BUY 挂单占比、`estimated_reward_per_100_usd_day`、退出深度、退出滑点、样本数和 midpoint 波动。`low_competition_mode=observe` 会把指标写入 quote plan 但强制低竞争 bucket 不可挂，且无开放订单/持仓的 observe-only 计划不会进入 AI/info-risk provider 请求；`enforce` 要求低竞争 competition-share、资金占比、退出和稳定性指标达标、`ai_advisory_enabled=true` 且 `info_risk_enabled=true/info_risk_mode=enforce`，之后仍走现有 AI advisory / info-risk cache gate，缺缓存或 provider 拒绝会 fail closed；低竞争 gate 因缺 fresh midpoint、盘口指标或历史样本不足而失败时会保留 quote legs 和 `orderbook_token_ids`，reason 使用 `low-competition data unavailable`，避免后续误报缺 YES/NO token；通过低竞争 gate 和 pre-LLM gate 的低竞争 pre-AI 市场会被标记为 low-competition 类型，再与 standard 候选按约 2:1 混排进入每批 10 个市场的临时盘口订阅和 AI 分析，info-risk refresh 使用独立队列补缓存；未通过低竞争 gate 的 condition 不会降级进入 standard 候选。AI/info-risk gate 完成后，worker 会把低竞争 observation 写入 `reward_low_competition_observations`，使用 gate 前固化的计划 notional、competition share 和资金占比，记录最终可挂状态、provider 拦截、样本不足、退出深度和滑点，API snapshot 再汇总最近 24 小时 shadow report；该 report 只给建议，不自动改配置。live placement 对低竞争 bucket 使用独立 `low_competition_max_markets`、`low_competition_max_open_orders` 和 `low_competition_max_position_usd`，并额外限制低竞争 open-like 订单最多约占全局 `max_open_orders` 的 30%（全局允许时至少 1 单），同时继续受全局订单/市场上限、kill switch、盘口风控、实际钱包余额和账户外部 BUY notional 约束。常规撤单、待提交 intent、last-look 和事件驱动 hard-risk 撤单都会用当前盘口/历史/账户重新计算低竞争 gate；低竞争待提交 BUY 在 POST 前会对同计划 `orderbook_token_ids` 和报价腿 token 做 1 秒 max-age last-look，缺任意必要盘口即 defer；事件路径会在订单自身或 companion token 更新时为当前低竞争 plan 的订单补齐同计划盘口，竞争度或资金占比硬失败会撤单，单纯 fresh book/history 数据不足不会只因该低竞争复核误撤。该实现仍只从数据库和 orderbook HTTP/内部 WS cache 读取数据，不直接调用 Polymarket Gamma/CLOB 外部 API。
...
新建挂单路径遇到盘口缺失、空盘口、超过 `stale_book_ms` 或已进入 placement freshness headroom 时等待 orderbook 订阅/缓存恢复，不写长期 skip；新建 quote intent 与已落库待提交 BUY 在提交前都会复用 live 撤单风控，并在真正 POST 前用 orderbook 服务做 1 秒 max-age last-look，风险不通过的本地 intent 会在提交前取消，last-look 缺盘口或刷新失败则 fail closed 等下轮重试。普通市场 BUY last-look 只要求当前提交 token；低竞争 BUY last-look 会要求同计划 `orderbook_token_ids` 和报价腿 token 全部由 orderbook 服务以 1 秒 max-age 返回，缺任意 token 即 defer。普通市场 BUY 仍检查计划 eligible、报价漂移、min depth、bid rank、depth drop、fill velocity、mass cancel、best ask touch、requote age 和 kill switch；低竞争 BUY 只共享通用硬风控（计划仍存在且 eligible、盘口缺失/为空、stale grace 后过期、best ask touch、kill switch 等），随后进入低竞争专用撤单 gate，不再执行普通市场的报价漂移、深度、排名、盘口历史突变或定期 requote 规则。live full tick、fast reconcile 和独立事件撤单 worker 都会读取开放订单/持仓活跃 token 的盘口。事件撤单 worker 只处理缺盘口、空盘口、SELL 盘口过期、BUY 非 stale 硬风险；普通市场 BUY 包含外部 bid 深度不足、bid rank 过高和盘口历史窗口风险，低竞争 BUY 则只跑低竞争专用 gate，且 companion token 更新会触发同 condition 复查。该路径不做订单/账户同步、重挂、退出提交、报价漂移换价或定期 requote；full/fast 路径仍覆盖完整检查。
...
旧的未提交 quote intent 会先经过当前计划、盘口、kill switch 和撤单风险检查，再允许提交；BUY intent 在真正 POST 前还会用 orderbook 服务做 1 秒 max-age last-look，若盘口缺失、刷新失败或出现 best ask touch 等硬风险则不提交；低竞争 BUY 会对同计划全部报价 token 做 last-look，缺任意 token 不提交并等下轮。任一提交结果未知、待最终对账或外部订单 404 会暂停全部新增买单，但不会阻断订单同步、风险撤单或卖出退出；外部订单 404 锁超过 5 分钟且仍无成交证据时会自动本地关闭；提交结果未知订单在恢复查询确认 CLOB 无对应 open 订单后经过 `LIVE_SUBMISSION_UNKNOWN_CLOSE_AFTER_SECS`（默认 600 秒）宽限也会自动本地关闭，不再需要手动改库，但若 positions 快照显示该 BUY token 在提交后出现库存则继续保留对账锁。同一批次第一笔 POST 结果未知后也不会继续发送后续买单。CLOB 已明确返回的 HTTP 4xx 拒单会将当前 intent 标记为 error，不会误进入提交未知锁。managed order 会持久化实际提交数量；SELL intent 的 price 保留非亏损退出 floor，post-only exit 被取消后的 replacement 会保留退出 floor 并在后续按 maker 规则重试，flatten replacement 保留退出 floor 并在后续按 best bid 非亏损 FAK 或继续等待。
...
- `run` 主循环包含 database-maintenance、register-orderbook-tokens、rewards、copytrade、news 和 execution/对账等任务；旧 arbitrage radar 与 signal recompute 循环已移除
...
- 活跃 rewards token 的 orderbook 更新会进入独立事件撤单 worker，按更新 token 过滤开放订单并立即执行 hard-risk cancel-only 检查；普通 BUY 硬风险包含计划不可挂、盘口缺失/为空、stale grace 后过期、best ask touch、外部 bid 深度不足、bid rank 过高、盘口历史窗口风险和全局 kill switch；低竞争 BUY 会额外补齐同计划 companion token 盘口，且 companion token 更新也会触发同 condition 低竞争订单复核，但只共享通用硬风控并进入低竞争专用撤单 gate，不再执行普通 depth/rank/history/requote 规则；该路径不跑订单/账户同步、重挂、退出提交、报价漂移换价或定期 requote，普通 fast reconcile 仍作为完整检查和周期兜底
...
- 低竞争市场 sleeve v2 已实现并默认关闭；启用后 worker 会构建独立低竞争候选 profile，并在 gate 前用 `rewards_low_competition_probe` 独立 source 按最多 10 个市场一批预热盘口和历史样本；低竞争候选后端强制关闭旧 liquidity/24h volume 过滤字段，按竞争程度/奖励密度而非流动性或成交量准入和排序，并可用低竞争专属 `quote_bid_rank`、安全边际、spread 和 market score 覆盖主策略报价/筛选参数。observe 模式只展示指标，enforce 模式通过默认 10U 探测资金竞争份额/竞争倍数、账户与单 condition 挂单资金占比、预估 reward/100/day、退出深度、入场退出滑点、坏成交恢复天数、盘口样本数、midpoint 波动、top-of-book 跳变和 AI/info-risk fail-closed gate 后才允许低竞争 bucket 下单；低竞争 enforce 可要求高置信度 AI `allow`，并可使用更严格的 info-risk avoid level。通过低竞争 gate 的 pre-AI 市场会在后台 AI advisory refresh 中与普通候选按约 2:1 混排进入每批 10 个市场的临时盘口订阅和 AI 分析，info-risk refresh 使用独立队列补缓存；full tick 会用 gate 前固化的计划 notional 和资金占比保存跨周期 observation，缺 fresh midpoint/盘口指标/历史样本时保留 quote legs 和 `orderbook_token_ids` 并标记 data unavailable，snapshot 提供包含坏成交恢复天数 P95 的 shadow report 和小额 enforce 建议，但不会自动启用；live placement 按 `low_competition_global_open_order_share_bps` 把低竞争 open-like 订单限制在全局 `max_open_orders` 的默认 30% 内（全局允许时至少 1 单）。低竞争撤单 gate 比入场 gate 宽松且阈值可配置：默认 competition share 低于入场阈值 80% 或 competition multiple 高于入场阈值 1.5 倍时，需要 30 秒前历史盘口同样确认才撤；账户/单 condition 挂单资金占比超限立即撤；退出深度阈值为 `max(20U, 当前 condition BUY 剩余 notional * 2)`，退出滑点超过 2c 才撤，滑点无法估算或历史样本不足不会单独触发撤单。
- rewards full tick 已读取 Gamma `markets.category` 作为候选评分输入；命中 `preferred_categories` 时只增加候选排序分，不绕过市场质量、盘口和风控硬过滤。AI advisory 已接入 full tick：live tick 只读已有 cache，后台 AI advisory provider refresh 与 info-risk provider refresh 两个 task 分别异步填充 `reward_market_advisories` 和 `reward_market_info_risks`，两条队列各自保留开放订单/持仓最高优先级，剩余普通 eligible/candidate 市场与通过 pre-LLM gate 的低竞争 pre-AI quote plan 按约 2:1 混排；新写入的 AI/info-risk cache 使用确定性 TTL jitter 打散过期时间，provider refresh 会在未过期缓存进入刷新窗口时提前续期；AI advisory 使用临时盘口批次补缓存，info-risk 独立批量/逐市场补缓存，`ai_advisory_batch_size` 和 `info_risk_batch_size` 分别控制主 refresh 单次 provider 请求包含的市场数，默认 1 保持逐市场，批量解析按 condition 拆分保存，漏项/错配或整体解析失败会回退单市场，provider 过载则停止对应 task 本轮剩余请求；每次实际外部 AI advisory / info-risk provider 调用会记录到 `llm_calls`，失败的 HTTP/解析结果也计入失败调用，供 snapshot 按日展示；AI 请求候选限定为 deterministic planner 原本 eligible 且仍缺少有效 advisory 的 condition，payload 将 5m price-history source candles 聚合为最多 24 根 1h candles 和摘要，cache key 只包含已完成小时级摘要、不包含当前小时内 source 更新；AI 开启后 provider 未配置、失败或缺缓存仍 fail closed，`avoid` 硬拦截；旧 `suitability` 响应按二值 fail-closed 兼容，`watch`/`avoid`/其它非 allow 值一律映射为 `avoid` 硬拦（旧 watch 放行行为已移除），低置信度 `allow` 在标准 sleeve 仍回退 deterministic 计划继续进入 live 盘口、资金和订单风控；高置信度 `allow` 只在 `selection_mode=enforce` 且 `quote_mode=auto` 时可把报价收窄为单腿，且不会放宽任何硬过滤；provider confidence 会被钳制到 `0..=1`。信息风险已接入缓存过滤：AI 开启时由 full tick 专用 info-risk provider refresh 推进，AI 未开启时仍可由独立 info-risk worker 扫描；live tick 只读缓存，enforce 模式下缺缓存仍 fail closed；新 condition 首次 BUY 报价还会按配置等待信息风险缓存和首单观察窗口，已有订单/持仓 condition 不受该首单 gate 限制；已有风险中只有 `critical`、官方结果、`resolution_imminent=true` 或配置为 `low/medium` 避免等级时命中的风险等级会硬拦截，普通 `high` 风险和仅 `risk_type=imminent_resolution` 但 `resolution_imminent=false` 的结果保留为信息提示并继续进入 live 盘口、资金和订单风控；新增买单会保守扣除未归属到本系统 managed order 的外部 BUY notional。
