...
| `worker/rewards/polling.rs` | rewards poll loop、盘口读取、事件驱动 fast reconcile、外部同步节流、5 天历史清理和进程内盘口历史 |
