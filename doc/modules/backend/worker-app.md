...
- rewards SELL 退出 intent 按非亏损 floor 提交 post-only maker SELL，不再用 FAK taker 卖出；提交前会按当前 token 持仓裁剪 size，无持仓 stale exit 会关闭，当前买一穿过 floor 时会按 30 秒退避等待可 resting 的盘口，不使用 midpoint 或页面“当前价”降价卖出。
