...
低竞争 rewards sleeve 目前仅有 [设计方案](../../rewards-low-competition-sleeve-plan.md)，worker 尚未实现 observe/enforce profile、`qualified_competition_usd`、预估 reward share、盘口稳定性窗口或独立小额度下单 gate。后续实现必须继续从 orderbook 服务 HTTP/内部 WS cache 和数据库读取数据，不允许在 rewards worker 中直接调用 Polymarket Gamma/CLOB 外部 API；observe 阶段不得改变现有 placement 行为。
...
- 低竞争市场 sleeve 尚未实现；当前 worker 不会按低竞争指标改变候选排序、注册额外 source、生成独立策略额度或执行低竞争专用下单。
