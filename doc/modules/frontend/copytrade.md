...
| `src/features/copytrade/components/smart-money-config-panel.tsx` | Smart Money 配置面板：启用/发现/advisory 开关、模式和基础阈值保存 |
...
| `src/features/copytrade/components/smart-money-signals-panel.tsx` | Smart Money 信号流面板：展示 recent_signals 的源价格、当前价格、滑点、状态和拒绝原因 |
...
- `updateSmartMoneyConfigAction` — 保存 Smart Money 配置（enabled、mode、discovery/advisory 开关和基础阈值）
...
- 启停跟踪配置保存
- Analyze 钱包分析命令入队
- Smart Money 配置保存：enabled、mode、候选发现/advisory 开关、样本/成交量/可跟分/滑点/深度/敞口阈值
...
- Smart Money 信号流展示：展示 recent_signals 的钱包、condition/token、方向、源成交价、当前价格、滑点、observe/rejected 状态、拒绝原因和生成时间；当前信号只用于观察和诊断，不代表可执行跟随订单
- 源成交列表和事件列表分页展示
