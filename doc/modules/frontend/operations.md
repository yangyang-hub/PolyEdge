...
页面当前仍通过确认对话框要求填写旧 step-up code，但后端已经改为 recent-auth session，header 内容不会授权操作。超过 recent-auth TTL 后页面尚不会自动弹出 `/auth/reauth`，这是当前 UX 缺口。只读用户的操作按钮尚未在此页面隐藏，但后端会拒绝写入。
