...
| `src/lib/contracts/dto/rewards.ts` + `dto/primitives.ts` | Rewards snapshot/plan DTO 与 event/fair-value enum |
| `src/lib/i18n/dictionaries/rewards.ts` | Rewards 页面文案、事件 provenance 摘要和 fair-value 状态文案 |
...
- Fair-value：`fair_value_*` 配置控制估值、最低 confidence、raw/effective trading edge、不确定性缓冲、LP rebate 展示折扣、YES/NO 中点偏离和历史窗口；工作台并列展示 effective 与 reward-adjusted edge，明确后者不参与 gate。工作台将 `not_evaluated` 单独展示，它不进入通过/拦截数和平均置信度分母，Runs ledger 也不会将其写成 `fair_value_passed=false`。
...
- 主策略页 header 提供 Fair value 入口；fair-value 页单独显示当前页 evaluated/pass/blocked/not-evaluated/avg confidence 指标和 quote plan 估值审计表，`not_evaluated` 不被渲染为 gate 失败。
...
- 报价计划默认展示通过非盘口依赖过滤且等待 live 盘口验证的候选，并按 maker `selection_score` 从高到低排序；`quote_readiness` 区分可报价、等待盘口、等待 AI/信息风险和已拦截。存在事件窗口 assessment 时，行内追加状态与 source/role/start-field/end-policy 审计摘要。
