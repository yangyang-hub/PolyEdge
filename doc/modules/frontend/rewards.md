...
`/rewards` 页面管理做市奖励机器人的生命周期：配置 validation/live 执行模式、向 worker 提交运行/取消/重置命令、查看订单/持仓/事件。
...
| `src/features/rewards/components/rewards-config-panel.tsx` | 分组策略配置面板（执行、市场筛选、报价构造、库存与验证） |
...
- **Run** → `runRewardBotOnce()` → API 写入 `run_once` 控制命令，worker 领取后按 `execution_mode` 执行一轮 validation 或 live 策略
- **Cancel open orders** → `cancelRewardBotOrders()` → API 写入 `cancel_all` 控制命令，worker 领取后撤销本地 validation 订单或 live 托管订单
- **Reset** → `resetRewardBot()` → API 写入 `reset` 控制命令，worker 领取后重置 validation 资金池和本地托管状态
- **Config 编辑** → `updateRewardBotConfig(patch)` → 即时更新配置
- **Execution mode**：`validation` 只验证挂单/成交/撤单/卖出等事件路径，不计提模拟 rewards 收益；`live` 通过后端 live executor 提交 post-only 订单
...
- 顶部执行概览展示 execution mode、启停/运行状态、市场就绪度、可用资金比例、最近扫描/运行时间和事件触发计数。
- 操作中心集中 Run / Save / Cancel / Reset，文案按 validation/live 提醒当前命令影响范围。
- 配置编辑按执行、市场筛选、报价构造、库存与验证分组，包含数值参数、布尔开关、`execution_mode` 与成交后策略。
- 配置编辑支持 `execution_mode=validation/live`；提示说明 `max_markets=0`、`max_open_orders=0`、`quote_size_usd=0` 都会停止新挂单。validation 开放买单软复用 `account_capital_usd`，单腿计划 notional 以 `min(quote_size_usd, account_capital_usd)` 为目标，但不再把 Qmin 计算展示为已赚收益。
...
- Positions 表格展示本地 rewards 库存账本；live 库存仍以后端真实成交对账缺口为准。
- 事件分类视图（挂单/撤单/吃单/奖励；validation 当前不产生奖励收益事件）
