...
- **成交后合并策略** → `balanced_merge_enabled` 默认关闭；开启后后端用独立 `balanced_merge_*` 市场/订单/edge/库存阈值追加 `balanced_merge` profile 候选，同一 condition 与标准策略冲突时标准策略优先。该 profile 固定 YES/NO 双边买单，要求两侧买价合计满足 edge；一侧成交后不生成 SELL、不撤对侧 BUY，full tick/fast reconcile 会自动发现两侧可配对库存并写入 merge intent。`balanced_merge_auto_execute_enabled` 默认关闭；开启后 worker 会用配置私钥作为 Safe owner 签名，通过 proxy wallet 提交 CTF `mergePositions` 链上交易。
...
- AI 建议面板保存 OpenAI-compatible/Anthropic provider、请求格式、advisory TTL、AI provider 并发开关、主/备模型最大并发、AI strategy hint 开关/最低置信度、信息风险启用、observe/enforce、过滤等级、信息风险 TTL、事件窗口配置、首单信息风险要求和首单观察窗口；GLM/DeepSeek/Agnes 不作为独立下拉项，而是通过 OpenAI-compatible base URL 和模型名配置，后端会在模型名包含 `glm`/`deepseek` 时改用 Chat Completions + JSON object，在模型名包含 `agnes` 时改用 Chat Completions + strict JSON schema。API key、base URL、模型名、请求超时和 web search 开关只来自 worker 环境变量，不会出现在前端配置或 snapshot；provider 并发开关和主/备并发上限属于前端可保存的 Rewards 业务配置。AI advisory 与信息风险扫描由 worker 的 combined provider refresh 覆盖当前候选，优先开放订单、持仓和可挂 quote plan；后端对同一 condition 使用一次 provider 调用同时返回到期的 advisory/info-risk section，不再提供多市场 batch 配置，并按主/备 endpoint 独立并发限制调度。事件窗口配置默认启用，默认只对 high confidence event time 执行 hard gate；StopNewQuotes 阻断新增 BUY，CancelOpenBuys/InEventWindow/PostEventCooldown 触发 BUY 撤单。报价计划 DTO 已包含 `event_window` assessment 和首单观察时间 `first_quote_observed_at`，当前表格主要通过 reason/readiness 展示阻断结果；AI advisory 和信息风险都只展示“允许挂单/不允许挂单”、confidence 和首条 reason/summary，并在 advisory metrics 含 `strategy_hint` 时显示 AI 建议方向、挡位和金额上限；信息风险 enforce 且缓存缺失时，后端会把对应计划显示为不可挂；首单 gate 只影响没有开放订单/库存的新 condition。
