...
| 文件 | 职责 |
|---|---|
| `dto.ts` | Barrel re-export，聚合 12 个 DTO 文件 |
...
| `news.ts` | 26 | 新闻数据 | GET |
| `funding.ts` | 23 | `readFundingStatus`、`submitFundingTransfer` | GET + POST |
...
- Funding DTO/API/action 已接入 `/api/v1/funding` 与 `/api/v1/funding/transfer`；前端只提交 token、amount、confirmed 和 `funding_transfer` step-up code，不提交 Polymarket 充值地址或任何私钥材料。
