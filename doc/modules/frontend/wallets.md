...
当前表单只收集 private key，未提供可选 CLOB API key/secret/passphrase 输入；后端 payload 格式已支持这些字段。默认关闭交易；页面不再收集无效 step-up code，危险写入仍由后端 recent-auth session 保护。页面尚未提供钱包编辑、secret rotation 或管理员查看他人钱包的专用筛选 UX。
