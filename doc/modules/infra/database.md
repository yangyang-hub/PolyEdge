...
- `wallet_import_contexts`：一次性浏览器混合加密导入 context；UUID 主键本身是 opaque context id，不再重复保存无读取方的 hash；签发使用 advisory-lock 串行容量检查并清理过期/已消费旧记录，消费按 owner + key id 原子完成；
...
- `wallet_equity_snapshots`：按用户/钱包保存当前操作性估值完整性、权益和 PnL snapshot；未接入生产者的逐仓位 valuation 预留表已从 clean baseline 删除。
...
- targeted orderbook 的周期 token 扫描使用 open-like managed order 与非零 position 的 partial token indexes；已删除被唯一约束覆盖的重复索引。
