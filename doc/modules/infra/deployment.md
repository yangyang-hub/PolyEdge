...
| 文件 | 职责 |
|---|---|
| `deploy/docker-compose.yml` | 双服务编排；wallet crypto 密钥经 `env_file` 注入 |
...
部署脚本要求 Linux/Bash 4.3+，并校验固定 server 监听、Postgres URL、`local|production`、production HTTPS public origin、exact CORS、真实 Argon2 hash、wallet crypto env 值（PEM 形态与 32-byte base64 storage key），以及空的生产前端 API base。Auto 模式使用文件锁、fast-forward、hash 增量构建和 `--remove-orphans`；`deploy/.env.server` 内容进入 server hash，修改密钥会触发重启。
