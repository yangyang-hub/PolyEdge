...
同源代理是生产 Cookie/CSRF 唯一路径。`NEXT_PUBLIC_POLYEDGE_API_BASE_URL` 必须留空；server 只在 Compose 内部网络暴露 38001，不映射宿主端口。

## 关键文件

| 文件 | 职责 |
|---|---|
| `deploy/docker-compose.yml` | 双服务编排与 RSA/storage key 只读挂载 |
...
| `deploy/.env.front.example` | 前端镜像/端口与必须为空的生产 API base |
...
- server 容器固定监听 `0.0.0.0:38001`，只通过 Compose `expose` 提供给前端 Nginx；`GET /healthz`、`GET /readyz`。
...
| `POLYEDGE_WALLET_CRYPTO__TRANSPORT_PRIVATE_KEY_PEM_FILE` | 容器内 PEM 路径，Compose 固定为 `/run/secrets/polyedge-wallet-import-private.pem` |
...
| `POLYEDGE_WALLET_STORAGE_KEY_FILE` | Compose 宿主 storage key 文件路径，默认 `deploy/secrets/polyedge-wallet-storage-key` |
| `POLYEDGE_WALLET_CRYPTO__STORAGE_KEY_FILE` | 容器内固定为 `/run/secrets/polyedge-wallet-storage-key`；内容经标准 base64 解码后精确 32 bytes |
...
RSA PEM 和 storage key 必须由 secret manager/受控宿主文件只读注入，Unix 文件权限必须阻止 group/world 访问（建议 `0600`），不得提交仓库、写入前端 public env 或打印到日志。部署脚本校验宿主文件、权限、固定容器路径和 storage key 长度；server 再校验 PEM、RSA 位数和密钥内容。当前 storage KEK 是单活动 key id，不是 KMS；没有在线旧 keyring 轮换。Compose 同时挂载 RSA PEM 与 storage key 文件。
...
部署脚本要求 Linux/Bash 4.3+，并校验固定 server 监听、Postgres URL、`local|production`、production HTTPS public origin、exact CORS、真实 Argon2 hash、wallet crypto 宿主文件/权限/容器路径、32-byte base64 storage key，以及空的生产前端 API base。Auto 模式使用文件锁、fast-forward、hash 增量构建和 `--remove-orphans`；RSA/storage key 文件内容也进入 server hash，同路径替换会触发重启。
...
- [ ] 修改 server env 时同步 `config.rs`、`.env.server.example`、`deploy.sh` 和本文件；Compose 固定端口与 secret mount 必须继续一致。
