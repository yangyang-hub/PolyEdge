...
| `POLYEDGE_REWARDS__AI_*` / `POLYEDGE_REWARDS__INFO_RISK_*` | `.env.api` | Rewards combined provider（AI advisory + 信息风险）的 key、base URL、模型、置信度、web search 和每轮 provider 上限等可选配置；主 provider 仍只配置 OpenAI-compatible 或 Anthropic，GLM/DeepSeek/Agnes 通过 OpenAI-compatible base URL 与模型名识别，fallback 同理；AI provider 单次请求默认超时 180 秒；AI advisory 事件驱动 batch 与 AI/info-risk batch-size 环境变量已移除 |
