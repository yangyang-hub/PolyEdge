...
V4 使用 Cookie session、RBAC、多租户 ownership、钱包 envelope、策略有效期和跨用户跟随。旧 Rewards、事件窗口、AI/provider、fair-value、Bearer/step-up、credential locator 与独立 worker/orderbook/replay 设计不在仓库中保留，也不是当前能力或兼容来源。
...
旧 `api-app.md`、`application.md`、`common.md`、`infrastructure.md`、`orderbook-app.md`、`worker-app.md`、`replay-app.md` 已删除，因为对应活动模块和独立进程在 V4 中不存在。
...
| [shared-components.md](frontend/shared-components.md) | `src/components/*` | 共享组件与控制台外壳 |
...
SELL exit、merge 和 Funding 不属于当前范围。当前 runtime 会写入非权威的操作性 fill 与 partial equity snapshot，但没有权威 venue fill ingestion 或完整 valuation/mark-to-market；不要把这些数据写成完整盈利核算能力。
...
| [deployment.md](infra/deployment.md) | `deploy/` + `scripts/` | `polyedge-server` + `polyedge-front` 同机或拆机部署 |
...
4. Git 历史中的旧设计不得覆盖根 `AGENTS.md` 与本目录的当前状态。
